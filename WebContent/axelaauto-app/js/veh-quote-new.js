...
function AddDiscountToJSON(discid, group_name, gsttype, item_id){
	...
	for(var i=0; i<configitems.length; i++){
		if(configitems[i].group_name === group_name && configitems[i].item_id === item_id){
			...
			if (gsttype === "state") {
				item_netprice = Math.ceil((((parseFloat(configitems[i].item_price) - discount) * parseFloat(configitems[i].item_tax1_rate)/100)
				...
				item_netamount = Math.ceil((((parseFloat(configitems[i].item_price) - discount) * parseFloat(configitems[i].item_tax1_rate)/100)
						+ ((parseFloat(configitems[i].item_price) - discount) * parseFloat(configitems[i].item_tax2_rate)/100)
						+ ((parseFloat(configitems[i].item_price) - discount) * parseFloat(configitems[i].item_tax4_rate)/100)
						+ (parseFloat(configitems[i].item_price) - discount)).toFixed(2));
				
				
				item_netamountwod = Math.ceil(((parseFloat(configitems[i].item_price) * parseFloat(configitems[i].item_tax1_rate)/100)
				...
				item_netprice =  Math.ceil((((parseFloat(configitems[i].item_price) - discount) * parseFloat(configitems[i].item_tax3_rate)/100)
						+ ((parseFloat(configitems[i].item_price) - discount) * parseFloat(configitems[i].item_tax4_rate)/100)
						+ (parseFloat(configitems[i].item_price) - discount)).toFixed(2));
				
				item_netamount = Math.ceil((((parseFloat(configitems[i].item_price) - discount) * parseFloat(configitems[i].item_tax3_rate)/100)
						+ ((parseFloat(configitems[i].item_price) - discount) * parseFloat(configitems[i].item_tax4_rate)/100)
						+ (parseFloat(configitems[i].item_price) - discount)).toFixed(2));
				
				item_netamountwod = Math.ceil(((parseFloat(configitems[i].item_price) *  parseFloat(configitems[i].item_tax3_rate)/100)
						+ (parseFloat(configitems[i].item_price) *  parseFloat(configitems[i].item_tax4_rate)/100)
						+ parseFloat(configitems[i].item_price)).toFixed(2));
...
function GetConfigurationDetails() {
	...
	GetConfigurationDetailsShowHint(
			"../axelaauto-app/app-veh-quote-new-check.jsp?get_config=yes&branch_id="
					+ branch_id
...
function CalculateExeDiscount(option_exediscount, addoffer_at_item_count) {
	if (option_exediscount == '1') {
		var exediscount = CheckNumeric(document
				.getElementById("txt_exediscount_at_id").value);
		var price = CheckNumeric(document.getElementById("txt_addoff_at_"
				+ addoffer_at_item_count + "_amt").value);

		if (parseFloat(price) > parseFloat(exediscount)) {
			document.getElementById("txt_addoff_at_" + addoffer_at_item_count
					+ "_amt").value = parseFloat(exediscount);
		}
...
function CalculateAfterTaxTotal(disc) {
	...
	clearTimeout(i);
	i = setTimeout('CalculateAfterTaxTotalTimeout("' + disc + '", "'
			+ total_disc + '")', 1000);
}
...
for(var i=1; i<=addoff_at_item_count; i++){
  ...
  if(addoff_at_item_check.checked==true){
	...
	console.log("addoff_at_disc_total==========="+addoff_at_disc_total)
  	...
  	console.log("addoff_at_disc_amt==========="+addoff_at_disc_amt)
...
function GroupValue(group_id, group_item_count, group_count, aftertax) {
	...
	var group_value = 0;
	var groups_count = parseInt(document.getElementById("txt_" + tax
			+ "group_count").value);
	console.log("GroupValue==222==");
	for (var j = 1; j <= groups_count; j++) {
		group_value = 0;
		group_item_count = document.getElementById("txt_" + tax + j + "_count").value;

		for (var i = 1; i <= group_item_count; i++) {
			console.log("GroupValue==444==");
			id = document.getElementById("chk_" + tax + j + "_" + i);
			item = id.checked;
			if (item == true) {
				value = parseFloat(document.getElementById("div_" + tax + j
						+ "_" + i + "_total").innerHTML);
				console.log("GroupValue==777==");
				group_value = parseFloat(group_value) + parseFloat(value);
			}
		}
	...
	if (group_id == '3') {
		multiselect_basetotal = document.getElementById("txt_" + tax
				+ "multiselect_basetotal").value;
		group_value = parseFloat(group_value)
				- parseFloat(multiselect_basetotal);
	}
...
function EvaluateTax(discount){
  ...
  var bt_totalwod = parseFloat(document.getElementById("txt_expricewod").value);
	console.log("bt_total==111=="+bt_total);
	console.log("bt_totalwod==111=="+bt_totalwod);
...
function CalculateDefault(group_id, group_item_count, count, group_count,
		aftertax) {
	...
	// if(aftertax=='0'){
	// var group_stock =
	// document.getElementById("txt_bt_gp_"+group_count+"_stock").value;
	...
	if (execute == '1') {
		...
		var new_value = parseFloat(document.getElementById("div_" + tax
				+ group_count + "_" + count + "_total").innerHTML);
		var base_value = "";
		for (var i = 1; i <= group_item_count; i++) {
			var selected_item = document.getElementById("chk_" + tax
					+ group_count + "_" + i).checked;
			if (selected_item == true) {
				base_value = document.getElementById("txt_" + tax + group_id
						+ "_basevalue").value;
				if (count != i) {
					document.getElementById("chk_" + tax + group_count + "_"
							+ i).checked = '';
				}
			}
		}
...
function CalculateMultiSelect(id, item_price, group_item_count, group_id,
		group_count, aftertax) {
	GroupValue(group_id, group_item_count, group_count, aftertax);
	CalculateTotal(aftertax);
}
