						...
						$
								.ajax({
									...
									success : function(data) {
										$('#dialog-modal')
												.html(
														'<iframe src="../account/account-contact-list.jsp?group=select_quote_contact" width="100%" height="100%" frameborder=0></iframe>');
									}
								});
...
function GetConfigurationDetails() {
	...
	var emp_quote_discountupdate = document
			.getElementById("emp_quote_discountupdate").value;
	showHint("../sales/veh-quote-check.jsp?get_config=yes&branch_id="
			+ branch_id + "&item_id=" + item_id + "&quote_date=" + quote_date
			+ "&emp_quote_discountupdate=" + emp_quote_discountupdate, 
			"config_details");
	showHint("../sales/veh-quote-check.jsp?branch_emp=yes&branch_id="
			+ branch_id,  "dr_executive");
}
...
function CalculateTotal(aftertax){
	...
	if(aftertax=='1'){
	  ...
	  if(addoff_bt_item_count!=0 && addoff_bt_item_count!=''){
		 ...
		 document.getElementById("txt_item_price").value = parseFloat(total)
			.toFixed(2);
		 document.getElementById("txt_total_disc").value = parseFloat(total_disc)
			.toFixed(2);
		 document.getElementById("div_item_price").innerHTML = parseFloat(total)
			.toFixed(2);
		 document.getElementById("txt_expricewod").value = parseFloat(totalwod)
			.toFixed(2);
		 clearTimeout(i);
...
//	        	alert("total_disc==calaftertaxtotal="+total_disc);
			...
			setTimeout('CalculateAfterTaxTotalTimeout("'+disc+'", "'+total_disc+'")', 1000);
			}
	  ...
	  function EvaluateTax(discount){
		  ...
		  for(var i=1; i<=at_group_count; i++){
			  ...
			  for(var j=1; j<=group_item_count; j++){
				...
				clearTimeout(i);
			  if(discount=="disc"){
				i = setTimeout('CalculateAfterTaxTotal("disc")', 600);
			  }else{
				i = setTimeout('CalculateAfterTaxTotal("")', 600);
			  }
		  }
