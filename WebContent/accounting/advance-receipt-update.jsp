...
<jsp:useBean id="mybean" class="axela.accounting.Advance_Receipt_Update" scope="request" />
...
<script type="text/javascript">
	...
	function advance() {
// 		alert("coming");
		if($("#dr_voucher_tax1").val()=='0'){
			$("#dr_voucher_tax2").prop("disabled","disabled");
		}
		CalculateTax();
	}
	
	function CalculateTax(){
		var tax1 = $("#dr_voucher_tax1").val();
		var tax2 = $("#dr_voucher_tax2").val();
		var rec_amt = $("#txt_main_party_amount").val();
		var tax1_amt = 0;
		var tax2_amt = 0;
		
		if($("#dr_voucher_tax1").val()!='0'){
			var tax1_rate = $("#dr_voucher_tax1").val().split("_")[1];
			tax1_amt = eval(rec_amt * tax1_rate / 100);
			$("#tax1").html("Tax1 Amt.: "+tax1_amt.toFixed(2))
		}
		
		if($("#dr_voucher_tax2").val()!='0'){
			var tax2_rate = $("#dr_voucher_tax2").val().split("_")[1];
			tax2_amt = eval(rec_amt * tax2_rate / 100);
			$("#tax2").html("Tax2 Amt.: "+tax2_amt.toFixed(2))
			tax1_amt = eval(tax1_amt + tax2_amt);
		}
		if($("#dr_voucher_tax1").val()=='0'){
			$("#tax1").hide();
		}else{
		...
		if(tax1_amt != 0){
			$("#total_tax").html("Total Tax: "+tax1_amt.toFixed(2))
		}
	}
...
// 												$('#dialog-modal-so')
// 														.html(
// 																'<iframe src="../sales/salesorder-list.jsp?group=select_so" height="100%" width="100%" frameborder=0></iframe>');
// 											}
// 										});
// 								$('#dialog-modal-so').dialog('open');
// 								return true;
// 							});
// 		});
// 		///
...
// 												$('#dialog-modal')
// 														.html(
// 																'<iframe src="../accounting/voucher-list.jsp?vouchertype_id=2&group=select_invoice" height="100%" width="100%" frameborder=0></iframe>');
// 											}
// 										});
...
// 												$('#dialog-modal')
// 														.html(
// 																'<iframe src="../customer/customer-contact-list.jsp?group=select_invoice_contact" height="100%" width="100%" frameborder=0></iframe>');
// 											}
// 										});
// 								$('#dialog-modal').dialog('open');
// 								return true;
			...
			<div class="page-content-inner">
				<div class="container-fluid">
					<ul class="page-breadcrumb breadcrumb">
						...
						</a> &gt; <a href="advance-receipt-update.jsp?<%=mybean.QueryString%>"><%=mybean.status%> <%=mybean.vouchertype_name%></a>:</li>
					</ul>
...
<!-- 												<div class="row"></div> -->
											<div class="form-element6">
												...
												<div class="form-element12">
													...
													<% if (!mybean.voucher_customer_id.equals("")) { %>
												...
												<div class='gst_tax'>
													<div class="form-element6">
															<label>Tax1<font color=red>*</font>: </label>
<!-- 																<select name="dr_voucher_tax1" id="dr_voucher_tax1" class="form-control"> -->
																	<span id="voucher_tax1"><%=mybean.PopulateTax(mybean.comp_id, mybean.voucher_tax1_id)%></span>
<!-- 																</select> -->
																<b><font id="tax1" color='#ff0000' ></font></b>
																
													</div>
													<div id="centraltax">
													<div class="form-element6">
															<label>Tax2<font color=red>*</font>: </label>
<!-- 																<select name="dr_voucher_tax2" id="dr_voucher_tax2" class="form-control"> -->
																<span id="voucher_tax2"><%=mybean.PopulateTax2(mybean.comp_id, mybean.voucher_tax2_id)%></span>
<!-- 																</select> -->
																<b><font id="tax2" color='#ff0000' ></font></b>
													</div>
													</div>
												</div>
												<div class='row'></div>
	...
	$('#ledger').on('change',function(){
	 ...
	 var url = '../accounting/ledger-check.jsp?ledgerstate=yes&customer_id='+$(this).val()+'&branch_id='+$("#dr_branch").val();
	...
	<script type="text/javascript">
		$(function() {
			...
			if($("#txt_gst_type").val()=="central"){
	        	$("#centraltax").hide()
	        }else{
	        	$("#centraltax").show()
	        }
			
			$("#voucher_tax1").on('change', function(){
				if($("#dr_voucher_tax1").val()== '0'){
					$("#dr_voucher_tax2").prop("disabled","disabled");
				}else{
					$("#dr_voucher_tax2").removeAttr("disabled");
				}
				CalculateTax();
			});
			$("#voucher_tax2").on('change', function(){
				CalculateTax();
				var vouchertax1=$("#voucher_tax1 option:selected").html().substring(0,4);
				var vouchertax2=$("#voucher_tax2 option:selected").html().substring(0,4);
				if($("#dr_voucher_tax2").val()=='0'){
					$("#tax2").hide();
					}
// 				alert("check==="+vouchertax1.includes(vouchertax2));
				if(vouchertax1.includes(vouchertax2)){
                showHint('../accounting/ledger-check.jsp?taxes2=yes&customer_id='+$("#ledger").val()+'&branch_id='+$("#dr_branch").val(), 'voucher_tax2');
                BootstrapSelect();
                setTimeout(function Populatetax(){ if($("#dr_voucher_tax2").val()=='0'){ $("#tax2").hide(); }},500);
				alert("Similar Taxes Not Found");
				}else{
           			$("#tax2").show();
           		}
			});
...
// 						alert("customer_id="+$(this).val());
                       if($(this).val()!='0'){
						...
						displaytaxes();
                        setTimeout(function Populatetaxes(){
                			if($("#txt_gst_type").val()=="central"){
                            	$("#centraltax").hide();
                            }else{
                            	$("#centraltax").show();
...
// 			            }
		...
		function displaytaxes(){
			showHint( '../accounting/ledger-check.jsp?taxes=yes&customer_id='+$("#ledger").val()+'&branch_id='+$("#dr_branch").val()+'&dr_voucher_tax1='+$("#dr_voucher_tax1").val()+'&dr_voucher_tax2='+$("#dr_voucher_tax2").val(), 'voucher_tax1');
			showHint( '../accounting/ledger-check.jsp?taxes2=yes&customer_id='+$("#ledger").val()+'&branch_id='+$("#dr_branch").val()+'&dr_voucher_tax1='+$("#dr_voucher_tax1").val()+'&dr_voucher_tax2='+$("#dr_voucher_tax2").val(), 'voucher_tax2');
		}
		</script>
