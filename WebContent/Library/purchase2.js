...
function SelectContact(contact_id, contact_name, customer_id, customer_name, customer_rateclass_id, customer_code) {
	...
	showHintAccount('../accounting/invoice-details2.jsp?cart_session_id='
...
function ShowNameHint() {
	...
	showHintAccount(
			'../accounting/ledger-check2.jsp?customer_name=' + name
...
function ShowMobileHint() {
	...
	showHintAccount(
			'../accounting/ledger-check2.jsp?contact_mobile=' + mobile
					+ '&cart_vouchertype_id=' + vouchertype_id
					+ '&search_customer=yes', 'invoice_details');
}
...
function ShowPhoneHint() {
	...
	// var session_id = document.getElementById("txt_session_id").value;
	showHintAccount(
			'../accounting/ledger-check2.jsp?contact_phone=' + phone
					+ '&cart_vouchertype_id=' + vouchertype_id
					+ '&search_customer=yes', 'invoice_details');
}
...
function ShowEmailHint() {
	...
	// var session_id = document.getElementById("txt_session_id").value;
	showHintAccount(
			'../accounting/ledger-check2.jsp?contact_email=' + email
					+ '&cart_vouchertype_id=' + vouchertype_id
					+ '&search_customer=yes', 'invoice_details');
}
...
function list_cart_items() {
	...
	if (voucher_so_id !='0'){
		showHintAccount('../accounting/invoice-details2.jsp?'
...
//		$("#"+idtr).html("<font color='#ff000'><b>Customer City Not Updated!</b></font>")
	}
}
...
function callAjax(i, value, cart_session_id, rateclass_id, config_inventory_current_stock, branch_id, location_id, item_serial_id) {
	...
	var idtr = "searchresult_"+i+"_tr";
	showHintAccount("../accounting/invoice-check2.jsp?"
			+ "row_no=" + i
			+ "&q=" + value
			+ "&cart_session_id=" + cart_session_id
			+ "&voucher_flow=purchase"
			+ "&rateclass_id=" + rateclass_id 
			+ "&config_inventory_current_stock=" + config_inventory_current_stock 
			+ "&branch_id=" + branch_id
			+ "&location_id=" + location_id 
			+ "&item_serial_id=" + item_serial_id , id);
	$("#"+idtr).show();
}
...
function ItemDetails(i,item_id, alt_qty, uom_id, price_amt, price_disc,
	...
	$('.searchresult').hide();
	var voucher_id = CheckNumeric(document.getElementById("txt_voucher_id").value);
	if (item_id != 0 && rateclass_id != 0) {
		var id ='itemindex_'+i;
		showHintAddItem('../accounting/invoice-details2.jsp?'
				+ 'cart_session_id=' + cart_session_id
				+ '&voucher_po_id='+voucher_id
				+ '&row_no=' + i
				+ '&voucher_flow=purchase'
				+ '&branch_id=' + branch_id
				+ '&gst_type=' + gst_type
...
//	var disc = parseFloat(CheckNumeric(document.getElementById('txt_item_price_disc_'+para).value));
//	var disc_per = parseFloat(CheckNumeric(document.getElementById('txt_item_price_dic_per_'+para).value));
...
//			caldiscpercent = caldiscpercent.toFixed(6);
//		}
//		document.getElementById('txt_item_price_disc_percent_add_'+para).value = caldiscpercent.toString();
//	} else {
//		if (emp_disc_update == 0) {
//			document.getElementById('txt_item_price_disc_'+para).value = disc_per.toString();
//			document.getElementById('txt_item_price_disc_percent_add_'+para).value = disc_per.toString();
//		} else {
...
//	var price = parseFloat(CheckNumeric(document.getElementById('txt_item_price_'+para).value));
//	var disc_per = parseFloat(CheckNumeric(document.getElementById('txt_item_price_dic_per_'+para).value));
//	var discpercent = parseFloat(CheckNumeric(document.getElementById('txt_item_price_disc_percent_add_'+para).value));
//	var emp_disc_update = parseInt(document.getElementById('txt_emp_invoice_discountupdate_'+para).value);
...
//	document.getElementById('uom_ratio').value = uom_ratio;
	...
	var price = parseFloat(CheckNumeric(document.getElementById('txt_item_price_'+para).value));
//	var unit_price = parseFloat(CheckNumeric(document.getElementById('txt_item_baseprice_'+para).value));
	...
	if (tax_rate1 != 0) {
		document.getElementById('txt_item_tax1_'+para).value = 
			parseFloat((((eval(unit_price) - eval(disc)) * eval(uom_ratio) * eval(tax_rate1 / 100))) * alt_qty).toFixed(4);
...
//		 alert("tax_value1==="+tax_value1);
	...
	if (tax_rate2 != 0) {
		if ((price_tax2_after_tax1 == 1) && (tax_rate1 != 0)) {
			document.getElementById('txt_item_tax2_'+para).value = 
				parseFloat((((eval(unit_price) - eval(disc)) * eval(uom_ratio)) + eval(tax_value1)) * eval(tax_rate2 / 100)).toFixed(4);
			tax_value2 = CheckNumeric(document.getElementById('txt_item_tax2_'+para).value);
			// alert("tax_value2==if="+tax_value2);
		} else {
			document.getElementById('txt_item_tax2_'+para).value = 
				parseFloat((((eval(unit_price) - eval(disc)) * eval(uom_ratio) * eval(tax_rate2 / 100))) * alt_qty).toFixed(4);
			tax_value2 = CheckNumeric(document.getElementById('txt_item_tax2_'+para).value);
//			 alert("tax_value2=else=="+tax_value2);
	...
	if (tax_rate3 != 0) {
		if ((price_tax3_after_tax2 == 1) && (tax_rate1 != 0) && (tax_rate2 != 0)) {
			document.getElementById('txt_item_tax3_'+para).value = 
				parseFloat((((eval(unit_price) - eval(disc)) * eval(uom_ratio)) + eval(tax_value1) + eval(tax_value2))
							* eval(tax_rate3 / 100)).toFixed(4);
			tax_value3 = CheckNumeric(document.getElementById('txt_item_tax3_'+para).value);
			// alert("tax_value3==="+tax_value3);
		} else {
			document.getElementById('txt_item_tax3_'+para).value = 
				parseFloat((((eval(unit_price) - eval(disc)) * eval(uom_ratio) * eval(tax_rate3 / 100))) * alt_qty).toFixed(4);
			tax_value3 = CheckNumeric(document.getElementById('txt_item_tax3_'+para).value);
//			 alert("tax_value3==="+tax_value3);
		}
	}
...
function GrandTotal(){
	...
	// this block is to calculate Round-Off if it is active in VoucherType-Update 
	if(eval(round_off)!=0){
		round_off_amt = (Math.round(GrandTotal.toFixed(4)) - GrandTotal.toFixed(4)).toFixed(4);
		if(eval(round_off_amt)!=0){
			var tax_data=$('#total_tax').html()+ "<div class='col-md-8'></div><label class='col-md-2 text-right' style='margin-top:8px;'>Round Off: </label><div class='col-md-2 text-right'><b><input type='text' name='txt_round_off_amount' id='txt_round_off_amount' class='form-control text-right' value='" + round_off_amt + "' readonly='readonly' /></b></div>";
...
//		---------------------------------------
				...
				if(tax_name.length!=0){
					...
					if(j == tax_name.length){
						tax_name[j]=$("#"+input_id).val().replace('.','');
...
//		-----------------------------
	...
	for(var k = 0;k<tax_name.length;k++){
		tax_data = tax_data + "<div class='col-md-8'></div><label class='col-md-2 text-right' style='margin-top:8px;'>"+tax_name[k]+": </label><div class='col-md-2 text-right'><b><input type='text' name='"+tax_name[k]+"' id='"+tax_name[k]+"' class='form-control text-right' value='" + tax_total_value[k].toFixed(4) + "' readonly='readonly' /></b></div>";
	}
	$('#total_tax').html(tax_data);
}
...
function AddItemList(){
	for(var x=1 ; x<=5 ; x++){
		...
		// check box
		+ "<td valign=top align=center> <input class='itemlist' id='chk_itemlist_" + i + "'"
		+ " type='checkbox' name='chk_itemlist_" + i + "' />"
		+ "</td>"
		// Item Name
		+ "<td><input type='text' class='form-control item_name' onkeyup='InvoiceItemSearch(" + i + ");' id='txt_itemname_" + i + "'"
		+ " name='txt_itemname_" + i + "'"
		+ " value=''></td>"
		// QTY
		+ "<td><input type='text' class='form-control' id='txt_item_qty_" + i + "'"
		+ " name='txt_item_qty_" + i + "'"
		+ " value=''></td>"
		// UOM
		+ "<td><input type='text' class='form-control' id='txt_item_uom_" + i + "'"
		+ " name='txt_item_uom_" + i + "'"
		+ " value=''></td>"
		// Price
		+ "<td><input type='text' class='form-control' id='txt_itemprice_" + i + "'"
		+ " name='txt_itemprice_" + i + "'"
		+ " value=''></td>"
		// Discount
		+ "<td><input type='text' class='form-control' id='txt_itemdiscount_" + i + "'"
		+ " name='txt_itemdiscount_" + i + "'"
		+ " value=''></td>"
		// Tax
		+ "<td class='tax-row'><input type='text' class='form-control' id='txt_itemtax_" + i + "'"
		+ " name='txt_itemtax_" + i + "'"
		+ " value=''></td>"
		// Total
		+ "<td><input type='text' class='form-control' id='txt_itemtotal_" + i + "'"
		+ " name='txt_itemtotal_" + i + "'"
		+ " value=''></td>"
	
		+ "</tr>"
		+ "<tr class='searchresult' id='searchresult_" + i + "_tr' style='display: none;'><td colspan='8' id='searchresult_" + i + "'></td></tr>";
...
$(function() {
    $('#Hintclicktocall').on('hidden.bs.modal',function(){
    	...
    	showHintFootable('../accounting/invoice-details.jsp?so=yes&cart_session_id='+cart_session_id
    			+'&cart_vouchertype_id='+vouchertype_id
    			+'&cart_voucher_id='+voucher_id
