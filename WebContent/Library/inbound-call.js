	function check(type) {
			...
			var call_callid = $("#call_callid").val();
			...
			if(type == 'enquiry') {
				url = '../inbound/enquiry-quickadd.jsp?inbound=yes&contact_id=' + contact_id + '&enquiry_call_id=' + call_id;
			} else if(type == 'preowned') {
				url = '../inbound/preowned-quickadd.jsp?inbound=yes&contact_id=' + contact_id + '&preowned_call_id=' + call_id;
			} else if(type == 'service') {
				url = '../inbound/booking-enquiry.jsp?contact_id=' + contact_id + '&vehfollowup_call_id=' + call_id;
			} else if(type == 'insurance') {
				url = '../portal/ecover-signin.jsp?contact_id=' + contact_id;
			} else if(type == 'ticket') {
				url = '../inbound/ticket-add.jsp?inbound=yes&add=yes&contact_id=' + contact_id + '&ticket_call_id=' + call_id;
			} else if(type == 'callback') {
				url = '../inbound/branch.jsp?call_id='+call_callid+'&customer_id=' + customer_id + '&contact_id=' + contact_id;
			} else if(type == 'canned') {
					...
					$.ajax({
						...
						success: function (data){
							console.log("data======"+data);
								if(data.trim() != 'SignIn'){
									if(data.trim() == 'SMS Sent Succesfully!'){
										$('#'+Hint).hide();
										Hint = 'success';
										$('#'+Hint).show();
	...
	function branchUpdate(page) {
		...
		var contact_id = $("#contact_id").val();
		var enquiry_call_id = $("#enquiry_call_id").val();
		var preowned_call_id = $("#preowned_call_id").val();
		var url = '';
		var hint = '';
		if(page == 'enquiry') { 
		 url = '../inbound/enquiry-quickadd.jsp?inbound=yes&dr_branch_id=' + branch_id + '&contact_id=' + contact_id + '&enquiry_call_id=' + enquiry_call_id;
		 hint = 'addenquiry';
		} else if(page == 'preowned') {
			url = '../inbound/preowned-quickadd.jsp?inbound=yes&dr_branch_id=' + branch_id + '&contact_id=' + contact_id + '&preowned_call_id=' + preowned_call_id;
			hint = 'addenquiry';
		}
		 $.ajax({
			url: url,
...
showHintUrl('../inbound/inbound-check.jsp?page=callback&executivesendsms=yes&call_id='+call_id+'&customer_id='+customer_id
		+'&contact_id='+contact_id+'&exe_id=' + exe_id+'&contact_name='+contact_name+'&contact_mobile=' + contact_mobile , 'addenquiry');
...
var call_no = $("#call_no").val();
var contact_name = $("#contact_name").val();
var url = '../inbound/executive-send-sms.jsp?contact_id=0&exe_id=' + emp_id + '&contact_mobile=' + call_no + '&contact_name=' + contact_name;
var Hint = 'addenquiry';
showHintUrl(url, Hint);
}
