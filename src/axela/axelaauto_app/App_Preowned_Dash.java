...
public class App_Preowned_Dash extends Connect {
		...
		try {
			...
			if (crs.isBeforeFirst()) {
				while (crs.next()) {
					...
					SOP("preowned_regdyear============" + preowned_regdyear);
					...
					if (!preowned_modified_id.equals("0")) {
						modified_by = ExecuteQuery("SELECT emp_name FROM " + compdb(comp_id) + "axela_emp WHERE emp_id=" + preowned_modified_id);
	...
	public String PopulateExecutive() {
		...
		try {
			StrSql = " SELECT emp_id, CONCAT(emp_name,' (',emp_ref_no,')') as emp_name"
					...
					+ " WHERE 1=1 AND emp_preowned=1 AND emp_active=1 "
	...
	public String PopulatePreownedPriority() {
		...
		try {
			...
			while (crs.next()) {
				...
				Str.append(">").append(crs.getString("prioritypreowned_desc")).append(" (").append(ConvertHoursToDaysHrsMins(crs.getDouble("prioritypreowned_duehrs"))).append(")</option>\n");
			}
			crs.close();
		} catch (Exception ex) {
			SOPError("Axelaauto===" + this.getClass().getName());
	...
	public String PopulateLostCase1(String comp_id) {
		...
		StringBuilder Str = new StringBuilder();
		try {
			StrSql = "SELECT preownedlostcase1_id,  preownedlostcase1_name" + " FROM "
					+ compdb(comp_id) + "axela_preowned_lostcase1"
					+ " WHERE 1=1" + " ORDER BY preownedlostcase1_name";
			// SOP("strstql1=======" + StrSql);
			CachedRowSet crs = processQuery(StrSql, 0);
	...
	public String PopulateLostCase2(String comp_id) {
		...
		try {
			StrSql = "SELECT preownedlostcase2_id, preownedlostcase2_name"
					+ " FROM "
					+ compdb(comp_id) + "axela_preowned_lostcase2"
					+ " WHERE 1=1" + " ORDER BY preownedlostcase2_name";
			CachedRowSet crs = processQuery(StrSql, 0);
			Str.append("<option value=0>Select</option>");
			while (crs.next()) {
				Str.append("<option value=")
						.append(crs.getString("preownedlostcase2_id")).append("");
				Str.append(StrSelectdrop(crs.getString("preownedlostcase2_id"), preowned_preownedlostcase2_id));
				Str.append(">").append(crs.getString("preownedlostcase2_name"))
						.append("</option>\n");
			}
			crs.close();
			return Str.toString();
	...
	public String PopulateLostCase3(String comp_id) {
		...
		StringBuilder Str = new StringBuilder();
		try {
			StrSql = "SELECT preownedlostcase3_id, preownedlostcase3_name" + " FROM "
					+ compdb(comp_id) + "axela_preowned_lostcase3"
					+ " WHERE 1=1" + " ORDER BY preownedlostcase3_name";
			CachedRowSet crs = processQuery(StrSql, 0);
			Str.append("<option value=0>Select</option>");
			while (crs.next()) {
				Str.append("<option value=")
						.append(crs.getString("preownedlostcase3_id")).append("");
				Str.append(StrSelectdrop(crs.getString("preownedlostcase3_id"), preowned_preownedlostcase3_id));
				Str.append(">").append(crs.getString("preownedlostcase3_name"))
						.append("</option>\n");
			}
			crs.close();
			return Str.toString();
