...
public class Stock_User_Import_Triumph extends Connect {
		...
		try {
			...
			if (columnLength != 10) {
				...
				for (j = 1; j < rowLength + 1; j++) {
					...
					for (h = 0; h < columnLength; h++) {
						if (h == 1) {
							...
							if (comp_id.equals("1009")) { // DD Motors //
								if (vehstock_branch_id.equals("1") || vehstock_branch_id.equals("2") ||
										vehstock_branch_id.equals("3") || vehstock_branch_id.equals("4")) {
									vehstock_vehstocklocation_id = "1";
								} else if (vehstock_branch_id.equals("5")) {
									vehstock_vehstocklocation_id = "11";
								} else if (vehstock_branch_id.equals("23")) { // One Triumph
									vehstock_vehstocklocation_id = "18";
								} else if (vehstock_branch_id.equals("50")) { // One Triumph - Gurugram
									vehstock_vehstocklocation_id = "27";
								}

							}
							else if (comp_id.equals("1011")) { // Indel
								if (vehstock_branch_id.equals("1")) {
									vehstock_vehstocklocation_id = "1";
								} else if (vehstock_branch_id.equals("2")) {
									vehstock_vehstocklocation_id = "6";
								} else if (vehstock_branch_id.equals("3")) {
									vehstock_vehstocklocation_id = "5";
								} else if (vehstock_branch_id.equals("5")) {
									vehstock_vehstocklocation_id = "4";
								} else if (vehstock_branch_id.equals("6")) {
									vehstock_vehstocklocation_id = "2";
								} else if (vehstock_branch_id.equals("7")) {
									vehstock_vehstocklocation_id = "3";
								}
							}
							else {
								vehstock_vehstocklocation_id = "1";
						...
						if (h == 3) {
							trans_option_id = CNumeric(ExecuteQuery("SELECT option_id FROM " + compdb(comp_id) + "axela_vehstock_option"
									+ " WHERE option_code = '" + sheetData[j][h] + "'"
									+ " AND option_brand_id = 151"));
						...
						if (h == 9) {
							// SOP("9");
							vehstock_delstatus_id = sheetData[j][h];
					...
					// SOP("errormsg==valid==" + errormsg);
					// SOP("vehstock_chassis_no----------" + vehstock_chassis_no);
					// SOP("vehstock_engine_no----------" + vehstock_engine_no);
					// SOP("vehstock_branch_id----------" + vehstock_branch_id);
					// SOP("vehstock_item_id----------" + vehstock_item_id);
					// SOP("vehstock_vehstocklocation_id -----------" + vehstock_vehstocklocation_id);
					if (errormsg.equals("") && !vehstock_chassis_no.equals("") && !vehstock_engine_no.equals("")
							&& !vehstock_branch_id.equals("0") && !vehstock_item_id.equals("0")) {
						...
						if (!vehstock_id.equals("0")) {
							...
							StrSql = "UPDATE " + compdb(comp_id) + "axela_vehstock"
									...
									// ConvertShortDateToStr(stock_arrival_date) +
									// "',"
									// + " vehstock_delstatus_id = " +
									// vehstock_delstatus_id + ","
					...
					if (!errormsg.equals("") && !branch_code.equals("")) {
						if (!vehstock_chassis_no.equals("")) {
