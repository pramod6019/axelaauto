...
public class Inventory_Cat_List extends Connect {
	// ////// List page links
	...
	private String cat_parent_id = "0";
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (!comp_id.equals("0")) {
				...
				cat_parent_id = CNumeric(PadQuotes(request.getParameter("cat_parent_id")));
	...
	public String Listdata() {
		...
		StrSql = "SELECT cat_id, cat_name, cat_parent_id, count(item_id) AS itemcount";

		StrJoin = " FROM " + compdb(comp_id) + "axela_inventory_cat"
				+ " LEFT JOIN " + compdb(comp_id) + "axela_inventory_item on item_cat_id = cat_id"
				+ " WHERE 1 = 1";
		if (cat_parent_id.equals("0")) {
			StrJoin += " AND cat_parent_id = 0";

		} else {
			StrJoin += " AND cat_parent_id = " + cat_parent_id;
		}
		...
		if (TotalRecords != 0) {
			...
			try {
				...
				Str.append("<tbody>\n");
				while (crs.next()) {
					LinkAddPage = "<a href=inventory-cat-update.jsp?Add=yes&cat_parent_id=" + crs.getString("cat_parent_id") + ">Add New Category...</a>";
					count = count + 1;
					Str.append("<tr>\n");
					Str.append("<td align=\"center\">").append(count).append("</td>\n");
					Str.append("<td align=\"left\">").append(crs.getString("cat_id")).append("</td>\n");
					Str.append("<td align=\"left\">").append(crs.getString("cat_name"));
					Str.append("</td>");
					...
					Str.append("<div class='dropdown' style='display: block'><center><div style='right: 4px;' class='btn-group pull-right'><button type=button style='margin: 0' class='btn btn-success'>"
							...
							+ "<li role=presentation><a href=\"inventory-cat-update.jsp?Update=yes&cat_id=" + crs.getString("cat_id")
							+ " \">Update Category</a></li>"
							+ "<li role=presentation><a href=inventory-cat-list.jsp?cat_parent_id=" + crs.getString("cat_id")
							+ " \">List Child Category</a></li>"
							+ "<li role=presentation><a href=inventory-item-list.jsp?cat_id=" + crs.getString("cat_id")
							+ " \">List Items (" + crs.getString("itemcount") + ")</a></li></ul></div></center></div>");
					Str.append("</td>\n");
				}
				Str.append("</tr>\n");
				Str.append("</tbody>\n");
			...
			LinkAddPage = "<a href=inventory-cat-update.jsp?Add=yes&cat_parent_id=" + cat_parent_id + ">Add New Category...</a>";
			Str.append("<font color=red>No Categories found!</font>");
		}
		return Str.toString();
	}
}
