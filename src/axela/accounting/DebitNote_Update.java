...
public class DebitNote_Update extends Connect {
			...
			if (!comp_id.equals("0")) {
				...
				if (add.equals("yes")) {
					...
					if (!addB.equals("yes")) {
						...
						if (ReturnPerm(comp_id, "emp_acc_debit_note_add", request).equals("1")) {
							AddFields(request);
					...
					status = "Update";
					if (!updateB.equals("yes") && !deleteB
							.equals("Delete Debit Note")) {
					...
					} else if (updateB.equals("yes") && !deleteB
							.equals("Delete Debit Note")) {
						if (ReturnPerm(comp_id, "emp_acc_debit_note_edit", request).equals("1")) {
							GetValues(request, response);
					...
					} else if (deleteB.equals("Delete Debit Note")) {
						response.sendRedirect(AccessDenied());
						// GetValues(request, response);
						if (ReturnPerm(comp_id, "emp_acc_debit_note_delete", request)
								.equals("1")) {
	...
	protected void GetValues(HttpServletRequest request,
		...
		voucher_branch_id = CNumeric(PadQuotes(request.getParameter("dr_branch")));
		debitcustid = CNumeric(PadQuotes(request.getParameter("ledger1")));
		creditcustid = CNumeric(PadQuotes(request.getParameter("ledger2")));
	...
	protected void CheckForm(HttpServletRequest request) {
		...
		if (voucherdate.equals("")) {
			...
			if (isValidDateFormatShort(voucherdate)) {
				...
				if (Long.parseLong(voucher_date) > Long
					...
					msg +=
							"<br> Date must be less than or equal to Current Date!";
				}
			} else {
		...
		if (creditcustid.equals("0")) {
			msg += "<br>Select Credit To!";
		}
		if (!creditcustid.equals("0") && !debitcustid.equals("0")) {
			if (debitcustid.equals(creditcustid)) {
				msg += "<br> Debit To & Credit To Ledger cannot be same!";
			}
		}
		...
		if (vouchertrans_paymode_id.equals("2")) {
			...
			if (vouchertrans_chequedate.equals("")) {
				...
				vouchertrans_cheque_date =
						ConvertShortDateToStr(vouchertrans_chequedate);
			}
		...
		// voucher_no =
		// PadQuotes(request.getParameter("txt_voucherno_" + i + ""));
		// if (!CNumeric(
		// PadQuotes(request.getParameter("txt_amount_" + i + "")))
		// .equals("0")) {
		// voucherrecpt_amount =
		// Double.parseDouble(PadQuotes(request
		...
		// .equals("on")) {
		// msg +=
		// "<br>Allocated amount for Invoice No.:" + voucher_no
			...
			try {
				...
				if (voucherclass_acc.equals("1")) {
					CalcCurrentBalanceThread calccurrentbalancethread =
							new CalcCurrentBalanceThread(voucher_id, comp_id, debitcustid, vouchertype_id);
					Thread thread = new Thread(calccurrentbalancethread);
					thread.start();
					// CalcuateCurrentVoucherBal(comp_id, voucher_id);
				} // / mail sending
				if (config_email_enable.equals("1") && vouchertype_email_enable
						.equals("1") && vouchertype_email_auto.equals("1")
						&& comp_email_enable
								.equals("1")) {
				...
				if (config_sms_enable.equals("1") && vouchertype_sms_enable
						.equals("1") && vouchertype_sms_auto.equals("1")
						&& comp_sms_enable
								.equals("1")) {
	...
	protected void PopulateFields() {
		try {
			StrSql = "SELECT COALESCE(voucher_id, 0) AS voucher_id, voucher_amount, voucher_no,"
					...
					+ " (SELECT vouchertrans_customer_id FROM " + compdb(comp_id) + "axela_acc_voucher_trans "
					+ " WHERE"
					+ " vouchertrans_voucher_id = voucher_id AND vouchertrans_dc = 0) AS creditcustid,"
			...
			// SOP("StrSql===populate==1=" + StrSqlBreaker(StrSql));
			prepmap.clear();
			prepkey = 1;
			if (crs.isBeforeFirst()) {
				while (crs.next()) {
					...
					debitcustid = crs.getString("voucher_customer_id");
					voucher_contact_id = crs.getString("voucher_contact_id");
					creditcustid = crs.getString("creditcustid");
		...
		if (msg.equals("")) {
			try {
				...
				StrSql = "UPDATE " + compdb(comp_id) + "axela_acc_voucher" + " SET"
						...
						+ " voucher_customer_id= " + debitcustid + ","
				...
				vouchertrans_customer_id = ExecuteQuery("SELECT customer_id FROM  " + compdb(comp_id) + "axela_customer"
	...
	protected void DeleteFields(HttpServletRequest request) throws SQLException {
		if (msg.equals("")) {
			try {
				conntx = connectDB();
				conntx.setAutoCommit(false);
				...
				updateQuery("UPDATE  " + compdb(comp_id) + "" + compdb(comp_id) + "axela_acc_voucher SET voucher_active = 0 "
						+ " WHERE voucher_id = " + voucher_id + "");
				// UPDATE "+compdb(comp_id)+"the customer current balance
	...
	public String PopulatePayment() {
		...
		try {
			StrSql = "SELECT customer_ledgertype, customer_name"
					+ " FROM  " + compdb(comp_id) + " " + compdb(comp_id) + "axela_customer"
	...
	public void CustomerCurrentBalance(String voucher_customer_id)
			throws SQLException {
		try {
			StrSql = "UPDATE  " + compdb(comp_id) + " " + compdb(comp_id) + "axela_customer"
					+ " INNER JOIN  " + compdb(comp_id) + "" + compdb(comp_id) + "axela_acc_voucher ON voucher_customer_id = voucher_customer_id"
					+ " SET";
	...
	protected void PopulateConfigDetails(String comp_id) {
		try {
			StrSql =
					"SELECT vouchertype_name, vouchertype_ref_no_enable, vouchertype_ref_no_mandatory,"
							+ " vouchertype_terms,"
							...
							+ " LEFT JOIN  " + compdb(comp_id) + "axela_branch ON branch_id = "
							+ voucher_branch_id
							+ ""
	...
	protected void InsertPendingInvoice(HttpServletRequest request, String update) {
		...
		try {
			...
			for (int i = 0; i <= voucher_count; i++) {
				...
				voucherbal_amount = PadQuotes(request.getParameter("txt_amount_" + i + ""));
				// /Debit NOte
				checked = PadQuotes(request.getParameter("chk_" + i + ""));
