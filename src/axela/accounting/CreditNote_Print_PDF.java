...
public class CreditNote_Print_PDF extends Connect {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			CheckPerm(comp_id, "emp_acc_credit_note_access", request, response);
			if (!comp_id.equals("0")) {
				...
				if (ReturnPerm(comp_id, "emp_acc_credit_note_access", request).equals("1")) {
					CreditNoteDetails(request, response, voucher_id, BranchAccess, ExeAccess, "pdf");
				}
			}
		} catch (Exception ex) {
	...
	public void CreditNoteDetails(HttpServletRequest request, HttpServletResponse response, String payment_id, String BranchAccess, String ExeAccess, String purpose) throws IOException,
			DocumentException {
		try {
			...
			if (crs.isBeforeFirst()) {
				...
				if (purpose.equals("pdf")) {
					...
					writer = PdfWriter.getInstance(document, new FileOutputStream(CachePath() + "CreditNote_" + voucher_id + ".pdf"));
				}
				document.open();
				...
				while (crs.next()) {
					...
					msg = "Dear Sir\n";
					msg += "\nWe have credited to your Account Rupees. " + (int) crs.getDouble("voucher_amount") + "/- (Rupees "
				...
				response.sendRedirect("../portal/error.jsp?msg=Invalid CreditNote!");
			}
		} catch (Exception ex) {
			System.out.println("AxA Pro===" + this.getClass().getName());
			System.out.println("Error in " + new Exception().getStackTrace()[0].getMethodName() + ": " + ex);
