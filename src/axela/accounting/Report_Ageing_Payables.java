...
public class Report_Ageing_Payables extends Connect {
	...
	public String voucherclass_id = "12";
	...
	protected void GetValues(HttpServletRequest request,
		...
		customer_id = PadQuotes(request.getParameter("accountingsupplier"));
		...
		try {
			if (crs.isBeforeFirst()) {
				...
				while (crs.next()) {
					...
					Str.append("<tbody>\n");
					Str.append("<tr onmouseover='ShowCustomerInfo(" + crs.getString("voucher_id") + ")'"
							+ " onmouseout='HideCustomerInfo(" + crs.getString("voucher_id") + ");'"
							+ " style='height:200px'");
					Str.append("<td align='center' valign='top'>")
							.append(count).append("</td>\n");
					Str.append("<td align='center' valign='top'>").append(
							unescapehtml(strToShortDate(crs
									.getString("voucher_date")))).append(
							"</td>\n");
					Str.append("<td align='center' valign='top'>").append(
							"<a href='../accounting/voucher-list.jsp").append(
							unescapehtml("?voucher_id="
									+ crs.getString("voucher_id")
									+ "&vouchertype_id="
									+ crs.getString("vouchertype_id"))).append(
							"'>");
					Str.append(crs.getString("voucher_id"))
							.append("</a></td>\n");
					Str.append("<td align='center' valign='top'>").append(
							unescapehtml(crs.getString("vouchertype_prefix")
					...
					Str.append("<td align='left' valign='top'>")
							.append(unescapehtml("<a href='../accounting/ledger-list.jsp?customer_id="
									+ crs.getString("customer_id") + "'>"))
							.append(unescapehtml(crs.getString("customer_name")))
							.append("</a></td>\n");
					Str.append("<td align='left' valign='top'>");
					Str.append(unescapehtml(DisplayCustomer(
							crs.getString("voucher_id"),
							crs.getString("customer_id"),
					...
					Str.append("<td align='right' valign='top'>").append(
							unescapehtml(crs.getString("voucher_amount")))
							.append("</td>\n");
					Str.append("<td align='right' valign='top'>").append(
							unescapehtml(crs.getString("netdue"))).append(
							"</td>\n");
					Str.append("<td align='right' valign='top'>").append(
							unescapehtml(crs.getString("1-30Days"))).append(
							"</td>\n");
					Str.append("<td align='right' valign='top'>").append(
							unescapehtml(crs.getString(">30-60Days"))).append(
							"</td>\n");
					Str.append("<td align='right' valign='top'>").append(
							unescapehtml(crs.getString(">60-90Days"))).append(
							"</td>\n");
					Str.append("<td align='right' valign='top'>").append(
							unescapehtml(crs.getString(">90-120Days"))).append(
							"</td>\n");
					Str.append("<td align='right' valign='top'>").append(
							unescapehtml(crs.getString(">120Days"))).append(
							"</td>\n");
					Str.append("</tr>\n");
				}
				Str.append("<tr>");
				...
				Str.append("<td align='right'><b>").append(
						unescapehtml(deci.format(oppbalance))).append(
						"</b></td>\n");
				Str.append("<td align='right'><b>").append(
						unescapehtml(deci.format(pending))).append(
						"</b></td>\n");
				if (pending1 != 0) {
					Str.append("<td align='right'><b>").append(
							unescapehtml(deci.format(pending1))).append(
							"</b></td>\n");
				} else {
					Str.append("<td align='right'><b>").append("&nbsp;")
							.append("</b></td>\n");
				...
				if (pending2 != 0) {
					Str.append("<td align='right'><b>").append(
							unescapehtml(deci.format(pending2))).append(
							"</b></td>\n");
				} else {
					Str.append("<td align='right'><b>").append("&nbsp;")
							.append("</b></td>\n");
				...
				if (pending3 != 0) {
					Str.append("<td align='right'><b>").append(
							unescapehtml(deci.format(pending3))).append(
							"</b></td>\n");
				} else {
					Str.append("<td align='right'><b>").append("&nbsp;")
							.append("</b></td>\n");
				...
				if (pending4 != 0) {
					Str.append("<td align='right'><b>").append(
							unescapehtml(deci.format(pending4))).append(
							"</b></td>\n");
				} else {
					Str.append("<td align='right'><b>").append("&nbsp;")
							.append("</b></td>\n");
				...
				if (pending5 != 0) {
					Str.append("<td align='right'><b>").append(
							unescapehtml(deci.format(pending5))).append(
							"</b></td>\n");
				} else {
					Str.append("<td align='right'><b>").append("&nbsp;")
							.append("</b></td>\n");
	...
	public String AgeingByGroup() {
		StringBuilder Str = new StringBuilder();
		int count = 0;
		String strpurchasedue = "SELECT"
				+ " COALESCE(SUM(purchasedue.voucher_amount),0)-COALESCE(SUM(voucherbal_amount),0)"
				+ " FROM " + compdb(comp_id) + "axela_acc_voucher purchasedue"
				+ " LEFT JOIN " + compdb(comp_id) + "axela_acc_voucher_bal ON voucherbal_trans_id = purchasedue.voucher_id"
				+ " WHERE 1=1"
				+ " AND purchasedue.voucher_customer_id = purchase.voucher_customer_id AND purchasedue.voucher_vouchertype_id = purchase.voucher_vouchertype_id ";
		...
		StrSql = "SELECT voucher_id, voucher_customer_id, customer_id, customer_name,"
				...
				+ " WHERE invtrans.voucher_customer_id = purchase.voucher_customer_id),0) AS netdue,"
				+ " ("
				+ strpurchasedue
				+ ""
				+ " AND SUBSTR(voucher_delivery_date,1,8)<=SUBSTR("
				+ ConvertShortDateToStr(AddDayMonthYear(startdate, 30, 0, 0, 0))
				+ ",1,8)) AS '1-30days',"
				+ " ("
				+ strpurchasedue
				...
				+ ConvertShortDateToStr(AddDayMonthYear(startdate, 120, 0, 0, 0))
				+ ",1,8)) AS '>120days'"
				+ " FROM  " + compdb(comp_id) + "axela_acc_voucher purchase"
				+ " INNER JOIN  " + compdb(comp_id) + "axela_acc_voucher_type ON vouchertype_id = voucher_vouchertype_id"
				+ " INNER JOIN  " + compdb(comp_id) + "axela_customer ON customer_id = voucher_customer_id"
				// + " INNER JOIN  " + compdb(comp_id) + "axela_customer ON customer_id = voucher_customer_id"
				+ " INNER JOIN  " + compdb(comp_id) + "axela_customer_contact ON contact_id = voucher_contact_id"
			...
			if (crs.isBeforeFirst()) {
				...
				while (crs.next()) {
					...
					if (crs.getDouble("netdue") != 0) {
						...
						Str.append("<td align='right'>").append("&nbsp;")
								.append("</td>\n");
					}
