...
public class Advance_Receipt_Update extends Connect {
	...
	public String voucher_tax1_amt = "0";
	public String voucher_tax2_amt = "0";
	...
	// public String voucher_advance = "0";
		...
		try {
			...
			if (!comp_id.equals("0")) {
				...
				if (add.equals("yes")) {
						...
						if (ReturnPerm(comp_id, "emp_acc_receipt_add", request).equals("1")) {
							...
							if (!msg.equals("")) {
								...
								response.sendRedirect(response.encodeRedirectURL("voucher-list.jsp?" + "voucher_id=" + voucher_id + "&voucherclass_id=" + voucherclass_id + "&vouchertype_id="
										+ vouchertype_id + "&msg=Advance Receipt Added Successfully!"));
								// }
							}
						} else {
					...
					status = "Update";
					if (!updateB.equals("yes") && !deleteB.equals("Delete Adv Receipt")) {
						if (!voucher_id.equals("0")) {
							...
							&& !deleteB.equals("Delete Adv Receipt")) {
						GetValues(request, response);
							...
							if (!msg.equals("")) {
								...
								response.sendRedirect(response
										.encodeRedirectURL("voucher-list.jsp?"
												...
												+ "&msg=Advance Receipt Updated Successfully!"));
							}
						} else {
							response.sendRedirect(AccessDenied());
						}
					} else if (deleteB.equals("Delete Adv Receipt")) {
						response.sendRedirect(AccessDenied());
						// GetValues(request, response);
	...
	public void GetValues(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (vouchertype_defaultauthorize.equals("1") && status.equals("Add")) {
		...
		// SOP("voucher_tax1===" + voucher_tax1);
		if (!voucher_tax1.equals("0") && !voucher_tax1.equals("")) {
			voucher_tax1_id = voucher_tax1.split("_")[0];
			voucher_tax1_amt = deci.format((Double.parseDouble(voucher_amount) * (Double.parseDouble(voucher_tax1.split("_")[1]) / 100)));
		...
		if (!voucher_tax2.equals("0") && !voucher_tax2.equals("")) {
			...
			voucher_tax2_amt = deci.format((Double.parseDouble(voucher_amount) * (Double.parseDouble(voucher_tax2.split("_")[1]) / 100)));
		}
		vouchertrans_customer_amount = deci.format(Double.parseDouble(vouchertrans_customer_amount)
				- Double.parseDouble(voucher_tax1_amt)
				- Double.parseDouble(voucher_tax2_amt));
		// SOP("vouchertrans_customer_amount=====" + vouchertrans_customer_amount);

		// SOP("voucher_tax1===GetValues===" + voucher_tax1);
		// SOP("voucher_tax2===GetValues===" + voucher_tax2);
	...
	public void CheckForm(HttpServletRequest request) {
		...
		// SOP("status===checkform==" + status);
		// SOP("vouchertype_defaultauthorize==checkform===" + vouchertype_defaultauthorize);
		// SOP("vouchertype_authorize==checkform==" + vouchertype_authorize);
		// SOP("voucher_no===checkform=====" + voucher_no);
		...
		// SOP("voucher_tax1====" + voucher_tax1);
		// SOP("voucher_tax2====" + voucher_tax2);
		// SOP("gst_type====" + gst_type);
		if (gst_type.equals("central")) {
			if (voucher_tax1.equals("0")) {
				msg += "<br>Select Tax1!";
			}
		} else if (gst_type.equals("state")) {
			if (voucher_tax1.equals("0")) {
				msg += "<br>Select Tax1!";
			}
			if (voucher_tax2.equals("") || voucher_tax2.equals("0")) {
				msg += "<br>Select Tax2!";
			}
		}

		if (!voucher_tax1.equals("") && !voucher_tax2.equals("")) {
			if (voucher_tax1.equals(voucher_tax2)) {
				msg += "<br>Tax1 And Tax2 Should Not Be Similar!";
			}
		}
	...
	public void addReceiptItem() {
		try {
			...
			StrSql += " vouchertrans_reconciliation)" + " VALUES" + " ("
					+ " " + vouchertrans_voucher_id + ","
					+ " " + vouchertrans_customer_id + ","
					+ " " + CNumeric(deci.format(Double.parseDouble(voucher_tax1_amt) + Double.parseDouble(voucher_tax2_amt))) + ","
					+ " '" + 0 + "',"// vouchertrans_dc
					+ " " + CNumeric(vouchertrans_item_id) + ","
					+ " " + vouchertrans_paymode_id + ",";
			if (payment_id.equals("2") && vouchertrans_paymode_id.equals("2")) {
			...
			// this is for GSt tax2
			if (!voucher_tax2_id.equals("0")) {
				StrSql = "INSERT INTO " + compdb(comp_id) + " axela_acc_voucher_trans" + " ("
						+ " vouchertrans_voucher_id,"
						+ " vouchertrans_customer_id,"
						+ " vouchertrans_amount,"
	...
	public void PopulateFields() {
		try {
			StrSql = "SELECT COALESCE(voucher_id, 0) AS voucher_id, voucher_amount,"
					...
					+ " voucher_advance, voucher_active, voucher_notes,"
			...
			try {
				...
				if (voucher_active.equals("0")) {
					StrSql = "Update " + compdb(comp_id) + "axela_acc_voucher"
							+ " Set voucher_authorize = " + voucher_active
							+ " where voucher_active= " + voucher_active;
					stmttx.executeUpdate(StrSql);
				}

				// if vouchertype_defaultauthorize is active and voucher active is active then voucher_authorize set to 1
				else if (voucher_active.equals("1") && vouchertype_defaultauthorize.equals("1")) {
					StrSql = "Update " + compdb(comp_id) + "axela_acc_voucher"
							+ " Set voucher_authorize = " + vouchertype_defaultauthorize
							+ " where voucher_active= " + voucher_active;
					stmttx.executeUpdate(StrSql);
				}
				conntx.commit();
	...
	public void PopulateTaxes() {
		if (!voucher_customer_id.equals("") && !voucher_branch_id.equals("0")) {
			gst_type = GetGstType(voucher_customer_id, voucher_branch_id, comp_id);
			// SOP("gst_type==if receipt update====" + gst_type);
		}
		...
		PopulateTax(comp_id, voucher_tax1_id);
		PopulateTax2(comp_id, voucher_tax2_id);
	}
