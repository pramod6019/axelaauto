...
public class Ledger_Check2 extends Connect {
		...
		if (!comp_id.equals("0")) {
			if (!GetSession("emp_id", request).equals("")) {
				try {
					...
					if (addmultiledger.equals("yes")) {
						...
						if (rowupdate.equals("")) {
							...
							if (!flag.equals("")) {
								...
								StrHTML = DisplayMultiLedger(session_id, comp_id);
							}
						} else {
							if (!flag.equals("")) {
					...
					// delete multi ledger entry
					if (deletemultiledger.equals("yes")) {
						DeleteMultipleLedger(cart_id);
						StrHTML = DisplayMultiLedger(session_id, comp_id);
					}
					// end journal
	...
	public void DeleteMultipleLedger(String cart_id) {
		StrSql = "DELETE FROM " + compdb(comp_id) + "axela_acc_cart WHERE cart_id = " + cart_id
				+ " AND cart_vouchertype_id = 18"
				+ " AND cart_session_id = " + session_id;
		updateQuery(StrSql);
	}
	...
	public String DisplayMultiLedger(String session_id, String comp_id) {
		int count = 0;
		double total = 0.00;
		StringBuilder Str = new StringBuilder();
		StrSql = "SELECT cart_id, customer_id,"
				+ " customer_name, cart_amount, cart_dc"
				+ " FROM " + compdb(comp_id) + "axela_acc_cart"
				+ " INNER JOIN " + compdb(comp_id) + "axela_customer ON customer_id = cart_customer_id"
				+ " WHERE 1=1"
				+ " AND cart_vouchertype_id = 18"
				+ " AND cart_option_id = 1"
				+ " AND cart_session_id = " + session_id;
			...
			if (crs.isBeforeFirst()) {
				...
				while (crs.next()) {
					...
					Str.append("<a href=\"javascript:delete_cart_item(")
							...
							.append(");\">X</a>");
					Str.append("</td>");
					Str.append("<td align='left' width='50%'>"
							+ crs.getString("customer_name") + "</td>");
					if (crs.getString("cart_dc").equals("1")) {
						Str.append("<td align='right'>"
								+ " <input type='text' style=\"text-align:right;\" maxlength=\"10\""
								+ " id='dr_journal_" + crs.getString("cart_id") + "'"
								+ " name='dr_journal_" + crs.getString("cart_id") + "'"
								+ " value='" + df.format(crs.getDouble("cart_amount")) + "'"
								+ " onkeyup=\"toFloat('dr_journal_" + crs.getString("cart_id") + "');journalclear('dr_journal_" + crs.getString("cart_id") + "');\""
								+ " onchange=\"journalupdate('dr_journal_" + crs.getString("cart_id") + "');\""
								+ " class='form-control'>"
								+ " </td>");
						Str.append("<td align='right'>"
								+ " <input type='text' style=\"text-align:right;\" maxlength=\"10\""
								+ " id='cr_journal_" + crs.getString("cart_id") + "'"
								+ " name='cr_journal_" + crs.getString("cart_id") + "'"
								+ " value=''"
								+ " onkeyup=\"toFloat('cr_journal_" + crs.getString("cart_id") + "');journalclear('cr_journal_" + crs.getString("cart_id") + "');\""
								+ " onchange=\"journalupdate('cr_journal_" + crs.getString("cart_id") + "');\""
								+ " class='form-control'>"
								+ " </td>");
					} else {
						Str.append("<td align='right'>"
				...
				double total_cr = 0.0;
				while (crs.next()) {
					if (crs.getString("cart_dc").equals("0")) {
						total_cr += crs.getDouble("cart_amount");
					} else if (crs.getString("cart_dc").equals("1")) {
						total_dr += crs.getDouble("cart_amount");
					}
				}
				crs.close();
				Str.append("<tr>");
				Str.append("<td colspan='3' align='right'>").append("<b>Total</b>").append("</td>");
				Str.append("<td align='right'><b>").append(df.format(total_dr)).append("</b></td>");
				Str.append("<td align='right'><b>").append(df.format(total_cr)).append("</b></td>");
				Str.append("</tr>");
				Str.append("<tr>");
				Str.append("<td colspan='3' align='right'>").append("<b>Diff. Amount:</b>").append("</td>");
				if (total_cr > total_dr) {
					Str.append("<td align='right'><b>").append(df.format((total_cr - total_dr))).append("</b>");
					Str.append("<td align='right'><b>").append("&nbsp;").append("</b>");
					Str.append("<input type='hidden' id='txt_diff_dc' name='txt_diff_dc' value='" + (total_cr - total_dr) + "'></td>");
				} else {
					Str.append("<td align='right'><b>").append("&nbsp;").append("</b>");
					Str.append("<td align='right'><b>").append(df.format((total_dr - total_cr))).append("</b>");
					Str.append("<input type='hidden' id='txt_diff_dc' name='txt_diff_dc' value='" + (total_dr - total_cr) + "'></td>");
				}
				Str.append("</tr>");
				...
				// + " $(\"#ledger1\").change("
				// + " function() {"
				// + " showHint( '../accounting/ledger-check.jsp?journalcurrbalance=yes&customer_id=' + $(this).val(),"
				// + " 'div_dr_curr_bal1'); amountcheck(1);"
				// + " });"
				// + " "
				// + " $(\".journal\").on('focusout',function(){"
				// + " var txt_id = this.id;"
				// + " var msg = \"\";"
				// + " var str = \"\";"
				// + " var cart_id = txt_id.substring(11,txt_id.length);"
				// + " "
				// + " var dramount = document.getElementById(\"dr_\"+txt_id.substring(3,txt_id.length)).value;"
				// + " var cramount = document.getElementById(\"cr_\"+txt_id.substring(3,txt_id.length)).value;"
				// + " "
				// + " if (dramount == \"\" && cramount == \"\") {"
				// + " msg += \"\nEnter Cr OR Dr Amount\";"
				// + " } if (msg == \"\") {"
				// + " if (dramount != 0) {"
				// + " str = '&dramount=' + dramount"
				// + " } else {"
				// + " str = '&cramount=' + cramount"
				// + " }"
				// + " alert(str);"
				// + " showHint('../accounting/ledger-check.jsp?addmultiledger=yes&rowupdate=yes' + str + '&session_id=' + <%=mybean.session_id%> + '&cart_id=' + cart_id, 'display_ledger');"
				// + " // for reseting party ledger, contact , current balance and party amount"
				// + " $(\"#ledger1\").select2(\"val\", \"\");"
				// + " document.getElementById(\"txt_vouchertrans_amount_c1\").value = \"\";"
				// + " document.getElementById(\"txt_vouchertrans_amount_d1\").value = \"\";"
				// + " document.getElementById(\"div_dr_curr_bal1\").value = \"\"; "
				// + " } else {"
				// + " alert(msg);"
				// + " }"
				// + " });"
