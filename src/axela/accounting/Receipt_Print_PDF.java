...
public class Receipt_Print_PDF extends Connect {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			CheckPerm(comp_id, "emp_receipt_access", request, response);
			if (!comp_id.equals("0")) {
				...
				if (ReturnPerm(comp_id, "emp_receipt_access", request).equals("1")) {
					ReceiptDetails(request, response, voucher_id, BranchAccess, ExeAccess, "pdf");
				}
			}
		} catch (Exception ex) {
	...
	public void ReceiptDetails(HttpServletRequest request, HttpServletResponse response, String payment_id, String BranchAccess, String ExeAccess, String purpose) throws IOException,
			DocumentException {
		try {
			...
			if (crs.isBeforeFirst()) {
				...
				while (crs.next()) {
					...
					if (!po_id.equals("0")) {
						...
						msg += " towards " + config_customer_name + " ID " + crs.getString("voucher_customer_id") + " dated " + strToShortDate(crs.getString("voucher_date")) + ".\n\n\n";
					}
					...
					cell = new PdfPCell(new Phrase("Payment ID: " + unescapehtml(crs.getString("voucher_id")), normal_font));
					cell.setVerticalAlignment(Element.ALIGN_CENTER);
					cell.setFixedHeight(35);
					table.addCell(cell);

					cell = new PdfPCell(new Phrase("Payment No.: " + unescapehtml(crs.getString("voucher_no")), normal_font));
					cell.setVerticalAlignment(Element.ALIGN_CENTER);
					cell.setFixedHeight(35);
					table.addCell(cell);

					cell = new PdfPCell(new Phrase(unescapehtml(crs.getString("customer_name")
					...
					if (!po_id.equals("0")) {
						cell = new PdfPCell(new Phrase("PO Date: " + strToShortDate(unescapehtml(crs.getString("voucher_date"))), normal_font));
						cell.setVerticalAlignment(Element.ALIGN_CENTER);
						cell.setFixedHeight(35);
						table.addCell(cell);

						cell = new PdfPCell(new Phrase("PO ID: " + unescapehtml(crs.getString("voucher_id")), normal_font));
						cell.setVerticalAlignment(Element.ALIGN_CENTER);
						cell.setFixedHeight(35);
						table.addCell(cell);
					} else {
