...
public class Enquiry_Brochure_Email extends Connect {
					...
					if (!msg.equals("")) {
						...
						if (!attachment.equals("")) {
							...
							if (comp_email_enable.equals("1") && config_email_enable.equals("1")
								...
								SendEmail();
								response.sendRedirect("enquiry-list.jsp?enquiry_id=" + enquiry_id + "&msg=Email sent successfully!");
							} else {
	...
	protected void GetValues(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, SQLException {
		...
		try {
			...
			if (crs.isBeforeFirst()) {
				...
				while (crs.next()) {
					...
					Str.append("<td>\n");
					Str.append("<input type=\"checkbox\" id=chk_brochure").append(count);
					Str.append(" name=chk_brochure").append(count).append(" />");
					Str.append("<input type=\"hidden\" id=hid_brochure").append(count);
					...
					if (!crs.getString("item_id").equals("0") && !crs.getString("item_url").equals("")) {
						Str.append("<a href=remote.jsp?item_id=" + crs.getString("item_id") + " target=_blank>");
					...
					if (!crs.getString("brochure_value").equals("")) {
						...
						Str.append("<td><a href=../Fetchdocs.do?" + "enquiry_brochure_id=");
				...
				Str.append("'/><input name='send_button' type='submit' class='button btn btn-success' id='send_button' value='Send Email'/>");
	...
	public String PopulateModel() {
		...
		try {
			...
			// Str.append("<div class=\"col-md-3 col-xs-3\" style=\"\"> ");
			Str.append("<label class=\"control-label col-md-4 col-xs-12\"> Model: </label>");
			Str.append("<div class=\"col-md-5 col-xs-12\"><select name=\"dr_enquiry_model_id\" id=\"dr_enquiry_model_id\" class=\"form-control\" onChange=\"document.form1.submit()\" >");
			Str.append("</div>");
			Str.append("</div>");
			Str.append("");
			while (crs.next()) {
