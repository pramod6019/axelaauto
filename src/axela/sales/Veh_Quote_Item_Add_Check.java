...
public class Veh_Quote_Item_Add_Check extends Connect {
			...
			if (jsonObject.get("group_name").equals(group_name)) {
				...
				while (iterator.hasNext()) {
					...
					if (jsonObject.getString(itemname).equals(item_name)) {
						error_msg = "Error!<br>Item is already added!";
						break;
					}
		...
		try {
			...
			if (crs.isBeforeFirst()) {
				...
				while (crs.next()) {
					...
					Str.append("</td>");
					Str.append("<td valign=top align=center><span class='btn btn-success' onclick=\"AddItem(").append(crs.getString("item_id") + ",");
				...
				Str.append("<center><b><font color=#ff0000>No Items Found!</font></b></center>");
			}
			return Str.toString();
		} catch (Exception ex) {
			SOPError("Axelaauto===" + this.getClass().getName());
