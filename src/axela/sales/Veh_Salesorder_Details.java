...
public class Veh_Salesorder_Details extends Connect {
	...
	public void AddCartItem() {
		if (rowcount.equals("yes")) {
			cart_rowcount = ExecuteQuery("Select coalesce(max(cart_rowcount),0)+1 from " + compdb(comp_id) + "axela_sales_so_cart");
			cart_option_id = "0";
		} else {
			cart_rowcount = "0";
			if (cart_id.equals("0")) {
				cart_option_id = ExecuteQuery("Select coalesce(max(cart_rowcount),0) from " + compdb(comp_id) + "axela_sales_so_cart");
			} else {
				cart_option_id = ExecuteQuery("Select coalesce(cart_rowcount,0) from " + compdb(comp_id) + "axela_sales_so_cart"
						+ " where cart_id = " + cart_id + "");
			}
		}
		StrSql = "Insert into " + compdb(comp_id) + "axela_sales_so_cart"
				+ " (cart_emp_id,"
				+ " cart_session_id,"
				+ " cart_rowcount,"
				+ " cart_item_id,"
	...
	public void UpdateCartItem() {
		StrSql = "Select cart_rowcount from " + compdb(comp_id) + "axela_sales_so_cart"
				+ " where cart_id = " + cart_id + "";
		cart_option_id = ExecuteQuery(StrSql); // get all the configured items for that item by using the cart_rowcount

		StrSql = "Delete from " + compdb(comp_id) + "axela_sales_so_cart"
		...
		StrSql = "Update " + compdb(comp_id) + "axela_sales_so_cart"
				+ " SET"
	...
	public void DeleteCartItem() {
		try {
			cart_option_id = ExecuteQuery("Select cart_rowcount from " + compdb(comp_id) + "axela_sales_so_cart"
					+ " where cart_id = " + cart_id + "");
			StrSql = "Delete from " + compdb(comp_id) + "axela_sales_so_cart"
					+ " where cart_option_id = " + cart_option_id + "";
			updateQuery(StrSql);
	...
	public void GetConfigurationDetails() {
		...
		StrSql = "Select item_id, item_name, group_type, group_name, price_amt, opt.option_group_id, item_serial,"
				...
				+ " group by option_group_id) as groupitemcount, coalesce(cart_option_id,0) as cart_option_id,"
				...
				+ " left join " + compdb(comp_id) + "axela_sales_so_cart on cart_item_id = item_id"
			...
			if (crs.isBeforeFirst()) {
				...
				while (crs.next()) {
					...
					if (!group.equals(crs.getString("group_name"))) {
						...
						// SOP("mode==" + mode + " gp==" + crs.getString("cart_option_group"));
					...
					Str.append("<tr>\n");
					// SOP("cart_option_id=="+cart_option_id+" and "+crs.getString("cart_option_id"));
	...
	public void ListCartItems() {
		StringBuilder Str = new StringBuilder();
		Double so_grandtotal = 0.00, total = 0.00;
		Double so_discamt = 0.00, pricetax = 0.00;
		Double so_totaltax = 0.00, selling_price = 0.00;
		Double so_qty = 0.00;
		Double so_netamt = 0.00;
		int count = 0;
		StrSql = "Select crt.cart_id, crt.cart_item_id, item_name,"
				+ " item_type_id, crt.cart_qty, crt.cart_price, item_nonstock, price_amt,"
				+ " crt.cart_discount, crt.cart_tax_id, item_code, item_id,"
				+ " crt.cart_tax_rate, crt.cart_item_serial,"
				+ " coalesce(crt.cart_option_group,'') as cart_option_group, crt.cart_soitem_id,"
				+ " COALESCE(crt.cart_tax,'0') as cart_tax,"
				+ " COALESCE(tax_name,'') as tax_name, price_variable,"
				// + " item_laundry_process_id,"
				+ " coalesce((select COUNT(DISTINCT cart_id)"
				+ " from " + compdb(comp_id) + "axela_sales_so_cart cart"
				+ " where cart.cart_option_id = crt.cart_rowcount"
				+ " GROUP BY cart.cart_option_id),0) as opitemcount"
				+ " from " + compdb(comp_id) + "axela_sales_so_cart crt"
				...
				+ " left join " + compdb(comp_id) + "axela_customer ON customer_id = crt.cart_tax_id AND customer_id = 1"
		...
		try {
			...
			if (crs.isBeforeFirst()) {
				while (crs.next()) {
					count = count + 1;
					so_netamt = so_netamt + (crs.getDouble("cart_qty") * ((Double.parseDouble(crs.getString("cart_price"))) - Double.parseDouble(crs.getString("cart_discount"))));
					...
					so_grandtotal = so_grandtotal + total;
					so_discamt = so_discamt + (Double.parseDouble(crs.getString("cart_qty")) * Double.parseDouble(crs.getString("cart_discount")));

					if (crs.getString("item_nonstock").equals("0")) {
						so_qty = so_qty + Double.parseDouble(crs.getString("cart_qty"));
					}
					so_totaltax = so_totaltax + (Double.parseDouble(crs.getString("cart_tax")) * Double.parseDouble(crs.getString("cart_qty")));
					String item_name = crs.getString("item_name");
					if (!crs.getString("item_code").equals("")) {
						item_name = item_name + " (" + crs.getString("item_code") + ")";
					}
					Str.append("\n<tr valign=top");
					if (!crs.getString("opitemcount").equals("0") && status.equals("Update") && !crs.getString("cart_soitem_id").equals("0")) {
						Str.append(" onClick=\"PreventUpdate();\"");
					} else {
						Str.append(" onClick=\"ItemDetails(").append(crs.getString("item_id")).append(",");
			...
			Str.append("<td align=\"right\"><input type=\"hidden\" name=\"txt_so_totaltax\" id=\"txt_so_totaltax\" value=\"").append(df.format(so_totaltax)).append("\">");
			Str.append("<input type=\"hidden\" name=\"txt_so_netamt\" id=\"txt_so_netamt\" value=\"").append(df.format(so_netamt)).append("\">");
			Str.append("<input type=\"hidden\" name=\"txt_so_grandtotal\" id=\"txt_so_grandtotal\" value=\"").append(df.format(Math.ceil(so_grandtotal))).append("\"><b>")
					.append(df.format(Math.ceil(so_grandtotal))).append("</b></td>");
			Str.append("</tr>");
			Str.append("<tr><td align=\"right\" colspan=\"8\">Total Savings:</td>");
			Str.append("<td align=\"right\"><input type=\"hidden\" name=\"txt_so_discamt\" id=\"txt_so_discamt\" value=\"").append(df.format(so_discamt)).append("\">").append(df.format(so_discamt))
					.append("</td>");
			Str.append("</tr>");
			Str.append("<tr><td align=\"right\" colspan=\"8\">Net Quantity:</td>");
			Str.append("<td align=\"right\"><input type=\"hidden\" name=\"txt_so_qty\" id=\"txt_so_qty\" value=\"").append(df.format(so_qty)).append("\">").append(df.format(so_qty)).append("</td>");
			Str.append("</tr>\n");
			crs.close();
		} catch (Exception ex) {
			SOPError("Axelaauto== " + this.getClass().getName());
