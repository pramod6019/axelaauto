...
public class Report_SOB_SO_Dash extends Connect {
	...
	public String chart_data_total = "";
	...
	public String SOSummary() {
		...
		try {
			...
			StrSql = " SELECT sob_name, count(so_id) AS Total "
					...
					+ " INNER JOIN " + compdb(comp_id) + "axela_inventory_item on item_id = soitem_item_id AND soitem_rowcount > 0"
					...
					+ " GROUP BY sob_id "
					+ " ORDER BY Total desc";
			// SOP("enquirySummary===" + StrSqlBreaker(StrSql));
			...
			if (crs.isBeforeFirst()) {
				...
				while (crs.next()) {
					...
					Str.append("" + crs.getString("total") + "");
					Str.append("</td>");
					Str.append("</tr>");
				}
				Str.append("<tr>");
				Str.append("<td align=right><b>Total: </b>");
				Str.append("</td>");
				Str.append("<td align=right>");
				Str.append("<b>" + total + "</b>");
				Str.append("</td>");
				Str.append("</tr>");
				Str.append("</table>");
				// //// Build pie chart
				...
				while (crs.next()) {
					count++;
					StrChart.append("{'type':'" + crs.getString("sob_name") + "','total':" + crs.getString("Total") + "}");
					StrChart.append(",");
					chart_data_total = chart_data_total + crs.getInt("Total");
					// StrChart.append("['" + crs.getString("sob_name") + " (" + crs.getString("Total") + ")'," + crs.getString("Total") + "]");
					// StrChart.append(",");
				}
