...
public class Report_Registration_Penetration extends Connect {
		...
		try {
			...
			if (!comp_id.equals("0")) {
				...
				if (go.equals("Go")) {
					...
					if (msg.equals("")) {
						StrHTML = ListRegistrationPenetration();
					}
				}
			}
		} catch (Exception ex) {
	...
	public String ListRegistrationPenetration() {

		StringBuilder Str = new StringBuilder();
		StringBuilder StrHead = new StringBuilder();
		StrSql = "SELECT emp_id, CONCAT(emp_name,' (', emp_ref_no,')') AS emp_name, emp_active,"
				+ " COUNT(so_id) AS socount,"
				+ " SUM(IF(so_reg_perm_reg_no != '', 1, 0)) AS regnogivencount,"
				+ " SUM(IF(so_reg_perm_reg_no = '', 1, 0)) AS regnonotgivencount"
		...
		// SOP("Query-------ListRegistrationPenetration-----------" + StrSql);
		...
		try {
			...
			if (crs.isBeforeFirst()) {
				...
				Str.append("<th data-hide=\"phone\">Registration Done</th>\n");
				Str.append("<th data-hide=\"phone\">Registration Not Done</th>\n");
				Str.append("</tr>\n");
				Str.append("</thead>\n");
				Str.append("<tbody>\n");
				int count = 0;
				int total_so_count = 0, total_regnogiven_count = 0, total_regnonotgiven_count = 0;

				while (crs.next()) {
					count++;
					total_so_count += crs.getInt("socount");
					total_regnogiven_count += crs.getInt("regnogivencount");
					total_regnonotgiven_count += crs.getInt("regnonotgivencount");
					...
					Str.append("<td align='right'>").append(crs.getString("regnogivencount")).append("</td>");
					Str.append("<td align='right'>").append(crs.getString("regnonotgivencount")).append("</td>");
					Str.append("</tr>");
				}
				Str.append("<tr>");
				Str.append("<td></td>");
				...
				Str.append("<td align='right'><b>").append(total_regnogiven_count).append("</b></td>");
				Str.append("<td align='right'><b>").append(total_regnonotgiven_count).append("</b></td>");
				Str.append("</tr>");
				Str.append("</table>");
				// Str.append("</div>");
			} else {
