...
public class Report_Vehicle_Quote_Discount extends Connect {
	...
	public String ListData() {
		...
		try {
			StrSql = "SELECT"
					+ " quotediscount_id,"
					+ " SUM(quotediscount_requestedamount) AS quotediscount_requestedamount,"
					+ " quotediscount_authorize_emp_id,"
					+ " COALESCE (emp_name,'') AS authorize_emp,"
			...
			StrSql += " INNER JOIN " + compdb(comp_id) + "axela_emp ON emp_id = quotediscount_authorize_emp_id"
					+ " WHERE 1 = 1"
					+ " AND quotediscount_authorize_status = 1"
					+ " AND SUBSTRING(quotediscount_authorize_time,1,8) >= SUBSTR('" + starttime + "',1,8)"
					+ " AND SUBSTRING(quotediscount_authorize_time,1,8) <= SUBSTR('" + endtime + "',1,8)"
					+ StrSearch
					+ " GROUP BY quotediscount_authorize_emp_id"
					+ " ORDER BY authorize_emp ";
			...
			if (crs.isBeforeFirst()) {
				...
				Str.append("<th data-toggle=\"true\">Authorized By</th>\n");
				Str.append("<th data-hide=\"phone\">SO Retail Count</th>\n");
				Str.append("<th data-hide=\"phone\">Discount Limit</th>\n");
				Str.append("<th data-hide=\"phone\">Quote Authorize Amount</th>\n");
				Str.append("<th data-hide=\"phone\">Discount Balance</th>\n");
				...
				while (crs.next()) {
					...
					// Str.append("<td align=right>").append(IndDecimalFormat(deci.format(crs.getDouble("quotediscount_requestedamount")))).append("</td>");
					Str.append("<td align=right>").append("0").append("</td>");
					Str.append("<td align=right>").append("0").append("</td>");
					Str.append("<td align=right>").append("0").append("</td>");
					Str.append("<td align=right>").append("0").append("</td>");
					Str.append("</tr>");
