...
public class Veh_Quote_Details extends Connect {
	...
	public void AddCartItem() {
		if (rowcount.equals("yes")) {
			cart_rowcount = ExecuteQuery("Select coalesce(max(cart_rowcount),0)+1 from " + compdb(comp_id) + "axela_sales_quote_cart");
			cart_option_id = "0";
		} else {
			cart_rowcount = "0";
			if (cart_id.equals("0")) {
				cart_option_id = ExecuteQuery("Select coalesce(max(cart_rowcount),0) from " + compdb(comp_id) + "axela_sales_quote_cart");
			} else {
				cart_option_id = ExecuteQuery("Select coalesce(cart_rowcount,0) from " + compdb(comp_id) + "axela_sales_quote_cart"
	...
	public void UpdateCartItem() {
		StrSql = "Select cart_rowcount from " + compdb(comp_id) + "axela_sales_quote_cart"
				+ " where cart_id = " + cart_id + "";
		...
		StrSql = "Update " + compdb(comp_id) + "axela_sales_quote_cart"
				+ " SET"
	...
	public void DeleteCartItem() {
		try {
			cart_option_id = ExecuteQuery("Select cart_rowcount from " + compdb(comp_id) + "axela_sales_quote_cart"
					+ " where cart_id = " + cart_id + "");
			StrSql = "Delete from " + compdb(comp_id) + "axela_sales_quote_cart"
					+ " where cart_option_id = " + cart_option_id + "";
			updateQuery(StrSql);
	...
	public void GetConfigurationDetails() {
		...
		StrSql = "Select item_id, item_name, group_type, group_name, price_amt, opt.option_group_id, item_serial,"
				...
				+ " left join " + compdb(comp_id) + "axela_sales_quote_cart on cart_item_id = item_id"
	...
	public void ListCartItems() {
		StringBuilder Str = new StringBuilder();
		Double quote_grandtotal = 0.00, total = 0.00;
		Double quote_discamt = 0.00, pricetax = 0.00;
		Double quote_totaltax = 0.00, selling_price = 0.00;
		Double quote_qty = 0.00;
		Double quote_netamt = 0.00;
		int count = 0;
		StrSql = "Select crt.cart_id, crt.cart_item_id, item_name,"
				+ " item_type_id, crt.cart_qty, crt.cart_price,"
				+ " item_nonstock, price_amt, crt.cart_discount,"
				+ " crt.cart_tax_id, item_code, item_id, crt.cart_tax_rate,"
				+ " crt.cart_item_serial,"
				+ " coalesce(crt.cart_option_group,'') as cart_option_group,"
				+ " crt.cart_quoteitem_id,"
				...
				+ " from " + compdb(comp_id) + "axela_sales_quote_cart cart"
				+ " where cart.cart_option_id = crt.cart_rowcount"
				+ " GROUP BY cart.cart_option_id),0) as opitemcount"
				+ " from " + compdb(comp_id) + "axela_sales_quote_cart crt"
				...
				+ " left join " + compdb(comp_id) + "axela_customer on customer_id = crt.cart_tax_id AND customer_tax = 1"
		...
		try {
			...
			if (crs.isBeforeFirst()) {
				while (crs.next()) {
					count = count + 1;
					quote_netamt = quote_netamt + (crs.getDouble("cart_qty") * ((Double.parseDouble(crs.getString("cart_price"))) - Double.parseDouble(crs.getString("cart_discount"))));
					...
					quote_grandtotal = quote_grandtotal + total;
					quote_discamt = quote_discamt + (Double.parseDouble(crs.getString("cart_qty")) * Double.parseDouble(crs.getString("cart_discount")));
					...
					if (crs.getString("item_nonstock").equals("0")) {
						quote_qty = quote_qty + Double.parseDouble(crs.getString("cart_qty"));
					}
					quote_totaltax = quote_totaltax + (Double.parseDouble(crs.getString("cart_tax")) * Double.parseDouble(crs.getString("cart_qty")));
					String item_name = crs.getString("item_name");
					if (!crs.getString("item_code").equals("")) {
						item_name = item_name + " (" + crs.getString("item_code") + ")";
					}
					Str.append("\n<tr valign=top");
					if (!crs.getString("opitemcount").equals("0") && status.equals("Update") && !crs.getString("cart_quoteitem_id").equals("0")) {
			...
			Str.append("<td align=\"right\"><input type=\"hidden\" name=\"txt_quote_totaltax\" id=\"txt_quote_totaltax\" value=\"").append(df.format(quote_totaltax)).append("\">");
			Str.append("<input type=\"hidden\" name=\"txt_quote_netamt\" id=\"txt_quote_netamt\" value=\"").append(df.format(quote_netamt)).append("\">");
			Str.append("<input type=\"hidden\" name=\"txt_quote_grandtotal\" id=\"txt_quote_grandtotal\" value=\"").append(df.format(Math.ceil(quote_grandtotal))).append("\"><b>")
					.append(df.format(Math.ceil(quote_grandtotal))).append("</b></td>");
			Str.append("</tr>");
			Str.append("<tr><td align=\"right\" colspan=\"8\">Total Savings:</td>");
			Str.append("<td align=\"right\"><input type=\"hidden\" name=\"txt_quote_discamt\" id=\"txt_quote_discamt\" value=\"").append(df.format(quote_discamt)).append("\">")
					.append(df.format(quote_discamt)).append("</td>");
			Str.append("</tr>");
			Str.append("<tr><td align=\"right\" colspan=\"8\">Net Quantity:</td>");
			Str.append("<td align=\"right\"><input type=\"hidden\" name=\"txt_quote_qty\" id=\"txt_quote_qty\" value=\"").append(df.format(quote_qty)).append("\">").append(df.format(quote_qty))
					.append("</td>");
			Str.append("</tr>\n");
			crs.close();
		} catch (Exception ex) {
