...
public class Report_SOE_Dash extends Connect {
	...
	public String SearchURL = "report-soe-dash.jsp?";
	...
	public String EnquirySummary() {
		...
		try {
			...
			StrSql = " SELECT soe_id, soe_name AS SOE, " + StrStage
			...
			if (crs.isBeforeFirst()) {
				...
				crs.beforeFirst();
				while (crs.next()) {
					String soe_id = crs.getString("soe_id");
					Str.append("<tr>");
					count++;
					...
					for (int i = 2; i <= numberOfColumns; i++) {
						...
						if (i == 2) {
							...
							Str.append("&soe_id=" + soe_id + "&").append(multiSelect + "><b>").append(crs.getString(i)).append("</b></a></td>");
							...
							Str.append("&soe_id=" + soe_id + "&").append(multiSelect + "><b><font color=blue>").append(crs.getString(i)).append("</font></b></a></td>");
							...
							Str.append("&soe_id=" + soe_id + "&").append(multiSelect + ">").append(crs.getString(i)).append("</a></td>");
						...
						// SOP("NOOFCOLUMNS"+Str.toString());
							...
							Str.append("&soe_id=" + soe_id + "&status_id=" + i).append(multiSelect + ">").append(crs.getString(i)).append("</a></td>");
	...
	public String ConversionSummary() {
		...
		try {
			StrSql = "SELECT soe_id, soe_name, "
					...
					+ " GROUP BY enquiry_soe_id ) AS tblenquiry ON tblenquiry.enquiry_soe_id = soe_id "
					+ " LEFT JOIN ( "
					+ " SELECT "
					...
					+ " LEFT JOIN " + compdb(comp_id) + "axela_sales_team_exe ON teamtrans_emp_id = emp_id "
					+ " WHERE 1 = 1" + EnquirySearch
					+ " GROUP BY enquiry_soe_id ) AS tbltestdrive ON tbltestdrive.enquiry_soe_id = soe_id "
					+ " LEFT JOIN ( "
					+ " SELECT "
					...
					+ " GROUP BY enquiry_soe_id ) AS tblso ON tblso.enquiry_soe_id = soe_id "
					+ " GROUP BY soe_id"
					+ " ORDER BY soe_name ";
			...
			if (crs.isBeforeFirst()) {
				...
				Str.append("<th data-toggle=\"true\">SOE</th>\n");
				...
				while (crs.next()) {
					...
					Str.append("<td align=left><b>").append(crs.getString("soe_name")).append("</b></td>");

					Str.append("<td align=right><a href=").append(SearchURL).append("conversion=yes&name=enquiry");
					Str.append("&soe_id=" + crs.getString("soe_id") + "&").append(multiSelect + ">").append(crs.getString("enquiry")).append("</a></td>");
					...
					Str.append("&soe_id=" + crs.getString("soe_id") + "&").append(multiSelect + ">").append(crs.getString("testdrives")).append("</a></td>");
					...
					Str.append("&soe_id=" + crs.getString("soe_id") + "&").append(multiSelect + ">").append(crs.getString("booking")).append("</a></td>");
					...
					Str.append("&soe_id=" + crs.getString("soe_id") + "&").append(multiSelect + ">").append(crs.getString("deliveries")).append("</a></td>");
	...
	public String PreparePieChart() {
		...
		try {
			if (crs.isBeforeFirst()) {
				...
				while (crs.next()) {
					count++;
					// chart_data = chart_data + "['" + crs.getString("soe_name")
					// + " (" + crs.getString("Total") + ")'," +
					// crs.getString("Total") + "]";
	...
	private void SoeDetails(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			String soe_ids = PadQuotes(RetrunSelectArrVal(request, "soe_ids"));
