...
public class Report_Vehicle_Quote_Discount_Pending extends Connect {
		...
		try {
			...
			if (!comp_id.equals("0")) {
				...
				if (go.equals("Go")) {
					...
					if (!exe_id.equals("")) {
						StrSearch = StrSearch + " AND quotediscount_request_emp_id IN (" + exe_id + ")";
					}
					if (!model_id.equals("")) {
	...
	public String ListData() {
		...
		try {
			StrSql = "SELECT"
					...
					+ " COALESCE (authorize.emp_name,'') AS authorize_emp,"
					+ " quotediscount_authorize_time,"
					+ " quotediscount_authorize_status,"
			...
			if (!team_id.equals("")) {
				StrSql += " LEFT JOIN " + compdb(comp_id) + "axela_sales_team_exe on teamtrans_emp_id = quotediscount_request_emp_id "
						+ " LEFT JOIN " + compdb(comp_id) + "axela_sales_team on team_id = teamtrans_team_id ";
			}
			if (!model_id.equals("")) {
				StrSql += " INNER JOIN " + compdb(comp_id) + "axela_inventory_item ON item_id = quote_item_id"
						+ " INNER JOIN " + compdb(comp_id) + "axela_inventory_item_model ON model_id = item_model_id";
			}

			StrSql += " INNER JOIN " + compdb(comp_id) + "axela_emp request ON request.emp_id = quotediscount_request_emp_id"
					+ " INNER JOIN " + compdb(comp_id) + "axela_emp authorize ON authorize.emp_id = quotediscount_authorize_emp_id"
					+ " WHERE 1 = 1"
					+ " AND quotediscount_authorize_status != 1"
					+ " AND SUBSTRING(quotediscount_entry_date,1,8) >= SUBSTR('" + starttime + "',1,8)"
					+ " AND SUBSTRING(quotediscount_entry_date,1,8) <= SUBSTR('" + endtime + "',1,8)"
					+ StrSearch
					+ " GROUP BY quotediscount_id"
					+ " ORDER BY quotediscount_entry_date DESC ";
			...
			if (crs.isBeforeFirst()) {
				...
				Str.append("<th data-hide=\"phone\">Quote Amount</th>\n");
				Str.append("<th data-hide=\"phone\">Discount Amount</th>\n");
				Str.append("<th data-hide=\"phone\">Due By</th>\n");
				Str.append("</tr>");
				Str.append("</thead>\n");
				Str.append("<tbody>\n");
				...
				while (crs.next()) {
					...
					Str.append("<td align=left>").append(ExeDetailsPopover(crs.getInt("quotediscount_request_emp_id"), crs.getString("request_emp"), "")).append("</td>");
					Str.append("<td align=center>").append("<a href=veh-quote-list.jsp?quote_id=" + crs.getString("quote_id") + ">" + crs.getString("quote_id") + "</a>").append("</td>");
					Str.append("<td align=center>").append("<a href=veh-salesorder-list.jsp?so_id=" + crs.getString("so_id") + ">" + crs.getString("so_id") + "</a>").append("</td>");
					Str.append("<td align=right>").append(IndDecimalFormat(deci.format(crs.getDouble("quote_grandtotal")))).append("</td>");
					Str.append("<td align=right>").append("<a href=veh-quote-discount-list.jsp?quote_id=" + crs.getString("quote_id") + "  target=blank>"
							+ IndDecimalFormat(deci.format(crs.getDouble("quotediscount_requestedamount")))).append("</a></td>");
					Str.append("<td align=left>").append(ExeDetailsPopover(crs.getInt("quotediscount_authorize_emp_id"), crs.getString("authorize_emp"), "")).append("</td>");
					Str.append("</tr>");
				}
				Str.append("</tbody>\n");
				Str.append("</table>");
