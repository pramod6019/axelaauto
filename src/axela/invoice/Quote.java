...
public class Quote extends Connect {
	...
	public String ListLink = "<a href=../accounting/voucher-list.jsp?voucherclass_id=5&vouchertype_id=5&smart=yes>Click here to List Quotes</a>";
	...
	public String QuoteSummary(HttpServletRequest request) {
		...
		try {
			StrSql = " SELECT branch_id, concat(branch_name,' (', branch_code, ')') as branchname, "
					+ " count(voucher_id) as quotecount, coalesce(sum(voucher_amount),0) as total "
		...
		//		Str.append("<b>Quote Summary</b><br>");
				...
				while (crs.next()) {
					...
					Str.append("<td><a href=\"../portal/branch-summary.jsp?branch_id=").append(crs.getInt("branch_id")).append("\">").append(crs.getString("branchname"))
							.append("</a></td>\n");
					Str.append("<td>").append(crs.getString("quotecount")).append("</td>");
					Str.append("<td> ").append(IndFormat(crs.getString("total"))).append("</td>");
					Str.append("</tr>");
				}
				Str.append("<tr>");
				Str.append("<td><b>Total: </b></td>\n");
				Str.append("<td><b>").append(quote_count).append("</b></td>\n");
