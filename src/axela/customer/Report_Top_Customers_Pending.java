...
public class Report_Top_Customers_Pending extends Connect {
				...
				if (go.equals("Go")) {
					...
					if (!dr_branch_id.equals("0")) {
						StrSearch = StrSearch + " AND (emp_branch_id=" + dr_branch_id + " OR emp_id= 1 OR emp_id in (SELECT empbr.emp_id FROM "
								+ " " + compdb(comp_id) + "axela_emp_branch empbr WHERE " + compdb(comp_id) + "axela_emp.emp_id=empbr.emp_id "
	...
	public String TopCustomersPending() {
		...
		try {
			...
			if (crs.isBeforeFirst()) {
				...
				Str.append("<font color=red><b>No Top Customers Pending Found!</b></font>");
			}
			crs.close();
		} catch (Exception ex) {
			SOPError("Axelaauto== " + this.getClass().getName());
