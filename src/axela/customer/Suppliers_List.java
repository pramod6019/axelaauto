...
public class Suppliers_List extends Connect {

	public String LinkHeader = "";
	public String LinkExportPage = "supplier-export.jsp?smart=yes&target=" + Math.random() + "";
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (!comp_id.equals("0")) {
				...
				LinkAddPage = "<a href=customer-update.jsp?Add=yes&tag=" + url_tag + ">Add New " + tag + " ... </a>";

				if (msg.toLowerCase().contains("delete")) {
					StrSearch = " AND customer_id = 0";
				} else if ("yes".equals(all)) {
					msg = "Results for all " + tag + "s!";
					StrSearch = StrSearch + " AND customer_id > 0 AND customer_type = 2";
				...
				} else if (smart.equals("yes")) // for smart search
				{
					...
					if (!GetSession("supplierstrsql", request).equals("")) {
						StrSearch = StrSearch + GetSession("supplierstrsql", request);
					}
				}
				StrSearch += BranchAccess;
				SOP("StrSearch====" + StrSearch);
				if (!StrSearch.equals("")) {
					SetSession("supplierstrsql", StrSearch, request);
				}
	...
	public String Listdata() {
		...
		if (!msg.equals("")) {
			try {
				...
				if (!msg.equals("")) {
					StrSql = "SELECT customer_id, customer_name, customer_active, customer_open_bal, customer_curr_bal,"
					...
					if (TotalRecords != 0) {
						...
						PageURL = "suppliers-list.jsp?" + QueryString + "&PageCurrent=";
						...
						while (crs.next()) {
							...
							if (!crs.getString("customer_phone1").equals("")) {
								Str.append("<br>").append(SplitPhoneNoSpan(crs.getString("customer_phone1"), 4, "T", crs.getString("customer_id")))
										.append(ClickToCall(crs.getString("customer_phone1"), comp_id));
							}
							if (!crs.getString("customer_phone2").equals("")) {
								Str.append("<br>").append(SplitPhoneNoSpan(crs.getString("customer_phone2"), 4, "T", crs.getString("customer_id")))
										.append(ClickToCall(crs.getString("customer_phone2"), comp_id));
							}
							if (!crs.getString("customer_phone3").equals("")) {
								Str.append("<br>").append(SplitPhoneNoSpan(crs.getString("customer_phone3"), 4, "T", crs.getString("customer_id")))
										.append(ClickToCall(crs.getString("customer_phone3"), comp_id));
							}
							if (!crs.getString("customer_phone4").equals("")) {
								Str.append("<br>").append(SplitPhoneNoSpan(crs.getString("customer_phone4"), 4, "T", crs.getString("customer_id")))
										.append(ClickToCall(crs.getString("customer_phone4"), comp_id));
							}
							if (!crs.getString("customer_mobile1").equals("")) {
								Str.append("<br>").append(SplitPhoneNoSpan(crs.getString("customer_mobile1"), 5, "M", crs.getString("customer_id")))
										.append(ClickToCall(crs.getString("customer_mobile1"), comp_id));
							}
							if (!crs.getString("customer_mobile2").equals("")) {
								Str.append("<br>").append(SplitPhoneNoSpan(crs.getString("customer_mobile2"), 5, "M", crs.getString("customer_id")))
										.append(ClickToCall(crs.getString("customer_mobile2"), comp_id));
							}
							...
							if (!crs.getString("customer_email1").equals("")) {
								Str.append("<br><span class='customer_info customer_" + crs.getString("customer_id") + "'  style='display: none;'><a href=\"mailto:")
										.append(crs.getString("customer_email1")).append("\">");
								Str.append(crs.getString("customer_email1")).append("</a></span>");
							}
							if (!crs.getString("customer_email2").equals("")) {
								Str.append("<br><span class='customer_info customer_" + crs.getString("customer_id") + "'  style='display: none;'><a href=\"mailto:")
										.append(crs.getString("customer_email2")).append("\">");
								Str.append(crs.getString("customer_email2")).append("</a></span>");
							}
							...
							if (group.equals("selectcustomer")) {
								...
								update_info += "<li role=presentation><a href=# onClick=\"javascript:window.parent.SelectPOCustomer("
										+ crs.getString("customer_id") + ", '" + PadQuotes(crs.getString("customer_name"))
										+ "');\">Select Supplier</a></li>";
