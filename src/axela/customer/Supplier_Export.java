...
public class Supplier_Export extends Connect {
	...
	public String exportpage = "supplier-export.jsp";
		...
		try {
			...
			if (!comp_id.equals("0")) {
				...
				if (!GetSession("supplierstrsql", request).equals("")) {
					StrSearch = GetSession("supplierstrsql", request);
				}
	...
	public void CustomerDetails(HttpServletRequest request, HttpServletResponse response) {
		try {
			StrSql = "SELECT customer_id AS 'ID', branch_name AS Branch, customer_name AS Name,"
					...
					+ " customer_gst_no AS 'Customer GST No.',"
					+ " COALESCE(date_format(customer_gst_regdate,'%d/%m/%Y'),'') AS 'Customer GST regdate',"
					+ " customer_arn_no AS 'Customer ARN No.',"
					+ " COALESCE(itstatus_name,'') AS 'Status',"
					+ " customer_address AS Address,"
					+ " city_name AS City, customer_pin AS Pin, customer_landmark AS Landmark,"
					...
					+ " limit " + exportcount;
			// SOP("StrSql(11)===supplier===" + StrSqlBreaker(StrSql));
	...
	public String PopulatePrintOption() {
		String print = "";
		print = print + "<option value =SupplierDetails" + StrSelectdrop("CustomerDetails", printoption) + ">Supplier Details</option>\n";
		return print;
	}
}
