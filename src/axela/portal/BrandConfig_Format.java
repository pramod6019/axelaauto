...
public class BrandConfig_Format extends Connect {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (!comp_id.equals("0")) {
				...
				brandconfig_id = PadQuotes(request.getParameter("brandconfig_id"));
				...
				if ("Update".equals(updateB)) {
					...
					if (msg.equals("")) {
						UpdateFields();
						response.sendRedirect(response.encodeRedirectURL("managebrandconfig-update.jsp?update=yes&brandconfig_id="
								+ brandconfig_id + "&msg=Format details for " + status + " updated successfully!"));
					}
				}
			}
		} catch (Exception ex) {
	...
	protected void UpdateFields() {
		...
		try {
			StrSql = "UPDATE "
					+ compdb(comp_id) + "axela_brand_config Set "
					+ "" + FormatName + " = '" + format_desc + "' ";
			if (email.equals("yes")) {
				StrSql = StrSql + "," + subjectName + " = '" + subject + "'";
			}
			StrSql = StrSql + " WHERE brandconfig_id = " + brandconfig_id;
			updateQuery(StrSql);
		} catch (Exception ex) {
			SOPError("Axelaauto== " + this.getClass().getName());
			SOPError("Error in " + new Exception().getStackTrace()[0].getMethodName() + ": " + ex);
