...
public class Managebranchclass_Update extends Connect {
				...
				if ("yes".equals(add)) {
					if (!"yes".equals(addB)) {
						...
						if (!msg.equals("")) {
							...
							response.sendRedirect(response.encodeRedirectURL("managebranchclass.jsp?rateclass_id=" + rateclass_id + "&msg=Branch Class Added Successfully!"));
						}
					}
				}
				if ("yes".equals(update)) {
					if (!"yes".equals(updateB) && !"Delete Branch Class".equals(deleteB)) {
						PopulateFields(response);
					} else if ("yes".equals(updateB) && !"Delete Branch Class".equals(deleteB)) {
						GetValues(request, response);
						UpdateFields();
						if (!msg.equals("")) {
							msg = "Error!" + msg;
						} else {
							response.sendRedirect(response.encodeRedirectURL("managebranchclass.jsp?rateclass_id=" + rateclass_id + "&msg=Branch Class Updated Successfully!"));
						}
					} else if ("Delete Branch Class".equals(deleteB)) {
						GetValues(request, response);
						DeleteFields();
						if (!msg.equals("")) {
							msg = "Error!" + msg;
						} else {
							response.sendRedirect(response.encodeRedirectURL("managebranchclass.jsp?msg=Branch Class Deleted Successfully!"));
						}
					}
				}
			}
	...
	protected void GetValues(HttpServletRequest request,
		...
		rateclass_name = PadQuotes(request.getParameter("txt_branchclass_name"));
		...
		try {
			if (!rateclass_name.equals("")) {
				...
				if (crs.isBeforeFirst()) {
					msg = msg + "<br>Similar Branch Class Found!";
				}
				crs.close();
			}
		} catch (Exception ex) {
	...
	protected void PopulateFields(HttpServletResponse response) {
		try {
			...
			if (crs.isBeforeFirst()) {
				...
				response.sendRedirect(response.encodeRedirectURL("../portal/error.jsp?msg=Invalid Branch Class!"));
			}
			crs.close();
		} catch (Exception ex) {
			SOPError("Axelaauto===" + this.getClass().getName());
