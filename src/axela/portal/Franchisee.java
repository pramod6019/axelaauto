...
public class Franchisee extends Connect {
	...
	// public String reportURL = "../Franchisee_Report.do?target=" + Math.random();
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (!comp_id.equals("0")) {
				...
				CheckPerm(comp_id, "emp_franchisee_access", request, response);
				...
				if (!smart.equals("yes")) {
					if (msg.equals("")) {
						...
						StrHTML = FranchiseeSummary(request);
					}
				}
			}
		} catch (Exception ex) {
	...
	public String FranchiseeSummary(HttpServletRequest request) {
		String count = "";
		StringBuilder Str = new StringBuilder();
		try {
			Str.append("<table border=1 cellspacing=0 cellpadding=0 class=\"listtable\">");
			Str.append("<tr align=center>\n");
			Str.append("<th colspan=2>Franchisee Status</th>\n");
			Str.append("</tr>");
			Str.append("<tr align=center>\n");
			Str.append("<td align=center width=50%>Total</td>\n");
			count = ExecuteQuery("select count(franchisee_id) from " + compdb(comp_id) + "axela_franchisee where 1=1");
			Str.append("<td align=center><b>").append(count).append("</b></td>\n");
			Str.append("</tr>");
			Str.append("<tr align=center>\n");
			Str.append("<td align=center width=50%>Active</td>\n");
			count = ExecuteQuery("select count(franchisee_id) from " + compdb(comp_id) + "axela_franchisee where  franchisee_active = '1'");
			Str.append("<td align=center><b>").append(count).append("</b></td>\n");
			Str.append("</tr>");
			Str.append("<tr align=center>\n");
			Str.append("<td align=center>Inactive</td>\n");
			count = ExecuteQuery("select count(franchisee_id) from " + compdb(comp_id) + "axela_franchisee where  franchisee_active = '0'");
			Str.append("<td align=center><b>").append(count).append("</b></td>\n");
			Str.append("</tr>");
			Str.append("</table>");
			return Str.toString();
	...
	// printoption = "FranchiseeDetails";
	// }
	// String print = "";
	// print = print + "<option value = FranchiseeDetails" + StrSelectdrop("FranchiseeDetails", printoption) + ">Franchisee Details</option>\n";
	// return print;
	// }
	//
