...
public class SO_WaitingPeriodDays_Format extends Connect {
	...
	public String title = "";
	public String sowaitingperiod_id = "";
		...
		try {
			...
			if (!comp_id.equals("0")) {
				...
				if ("Update".equals(updateB)) {
					...
					if (msg.equals("")) {
						UpdateFields();
						response.sendRedirect(response.encodeRedirectURL("so-waitingperioddays-update.jsp?update=yes&sowaitingperiod_id=" + sowaitingperiod_id + "&msg=Format details for " + status
	...
	protected void PopulateFields() {
		...
		StrSql = StrSql + " FROM " + compdb(comp_id) + "axela_brand_sowaitingperioddays where sowaitingperiod_id=" + sowaitingperiod_id;
	...
	protected void UpdateFields() {
		...
		try {
			StrSql = "UPDATE " + compdb(comp_id) + "axela_brand_sowaitingperioddays Set "
					+ "" + FormatName + " = '" + format_desc + "' ";
			if (email.equals("yes")) {
				StrSql = StrSql + "," + subjectName + " = '" + subject + "'";
			}
			StrSql = StrSql + " where sowaitingperiod_id=" + sowaitingperiod_id;
			// SOP("SqlStr==" + StrSql);
			updateQuery(StrSql);
		} catch (Exception ex) {
			SOPError("Axelaauto== " + this.getClass().getName());
