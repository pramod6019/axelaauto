...
public class Branch_Format extends Connect {
		...
		try {
			...
			if (!comp_id.equals("0")) {
				...
				if ("Update".equals(updateB)) {
					...
					if (msg.equals("")) {
						UpdateFields();
						response.sendRedirect(response.encodeRedirectURL("branch-update.jsp?update=yes&branch_id=" + branch_id + "&msg=Format details for " + status + " updated successfully!"));
					}
				}
			}
		} catch (Exception ex) {
	...
	protected void PopulateFields() {
		...
		StrSql = StrSql + " from " + compdb(comp_id) + "axela_branch where branch_id=" + branch_id;
		try {
	...
	protected void UpdateFields() {
		...
		try {
			StrSql = "UPDATE " + compdb(comp_id) + "axela_branch Set "
