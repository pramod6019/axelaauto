...
public class MIS_Dash_Campaign extends Connect {
           ...
           if(!comp_id.equals("0"))
            {
                ...
                if (msg.equals("")) {
                    StrHTML = EnquiryCampaignSummary();
                }
            ...
            SOP("Main Logic Error Home_Intro: " + ex);
        }
    }
...
//
//        group_id = RetrunSelectArrVal(request, "dr_group");
...
//
    ...
    public String EnquiryCampaignSummary() {
        ...
        try {
            StrSql = " SELECT concat(campaign_name,' <br>(',date_format(campaign_startdate,'%d/%m/%Y'),'-',date_format(campaign_enddate,'%d/%m/%Y'),')') as campaign, "
                    + " coalesce(campaign_budget,0) as budget, "
                    + "  (select count(student_id) "
                    + "  from " + compdb(comp_id) + "axela_student "
                    + "  where student_campaign_id = campaign_id and student_active='1') as total, "
                    + "  ( "
                    + "  select count(student_id) "
                    + "  from " + compdb(comp_id) + "axela_student "
                    + "  where student_campaign_id = campaign_id and student_no='' and student_active='1' and student_enqfollowup_active='0') as closedlost, "
                    + "  (select count(student_id) "
                    + "  from " + compdb(comp_id) + "axela_student "
                    + "  where student_campaign_id = campaign_id and student_enqfollowup_active='1' and student_no='' and student_active='1') as opencount, ( "
                    + "  select count(student_id) "
                    + "  from " + compdb(comp_id) + "axela_student "
                    + "  where student_no!='' and student_campaign_id = campaign_id and student_active='1') as closedwon,coalesce(( "
                    + "  select sum(invoice_grandtotal) "
                    + "  from " + compdb(comp_id) + "axela_student "
                    + "  inner join " + compdb(comp_id) + "axela_invoice on invoice_student_id = student_id "
                    + "  where student_no!='' and student_campaign_id = campaign_id and student_active='1' and invoice_active='1'),0) as invoicevalue,coalesce(( "
                    + "  select sum(receipt_total) "
                    + "  from " + compdb(comp_id) + "axela_student "
                    + "  inner join " + compdb(comp_id) + "axela_invoice on invoice_student_id = student_id "
                    + "  inner join " + compdb(comp_id) + "axela_invoice_receipt on receipt_invoice_id = invoice_id "
                    + "  where student_no!='' and student_campaign_id = campaign_id and student_active='1' and invoice_active='1' and receipt_active=1),0) as receiptvalue "
                    + "  from " + compdb(comp_id) + "axela_sales_campaign "
                    + "  inner join " + compdb(comp_id) + "axela_student on student_campaign_id = campaign_id "
                    + "  inner join " + compdb(comp_id) + "axela_branch on branch_id = student_branch_id "
                    + "  where 1=1 " + EnquirySearch
                    + "  group by campaign_id "
                    + "  order by total, campaign_name ";
...
//            SOP("strsql in LmsSummary---" + StrSqlBreaker(StrSql));
            ...
            Str.append("</tr>\n");
            while (crs.next()) {
                budget = budget + crs.getInt("budget");
                ...
                Str.append("<tr>\n");
                Str.append("<td valign=top align=left>" + crs.getString("campaign") + "</td>\n");
                Str.append("<td valign=top align=right>" + IndFormat(crs.getString("budget")) + "</td>\n");
            ...
            Str.append("<td align=right><b>" + IndFormat(budget + "") + "</b></td>\n");
            Str.append("<td align=right><b>" + total + "</b></td>\n");
            ...
            SOPError("Axelaauto===" + this.getClass().getName());
            SOPError("Error EnquiryCampaignSummary--:" + ex);
            return "";
        }
    }
...
//            SOP("StrSql==== in PopulateExecutive" + StrSqlBreaker(StrSql));
            ...
            SOPError("Axelaauto===" + this.getClass().getName());
            SOPError("Error PopulateExecutive: " + ex);
            return "";
        }
    }
...
//             SOP("StrSql in PopulateCountry==========" + StrSql);
            ...
            SOPError("Axelaauto===" + this.getClass().getName());
            SOPError("Error PopulateCourse: " + ex);
            return "";
        }
        return stringval;
    }
    ...
    public String RetrunSelectArrVal(HttpServletRequest request, String name) {
        ...
        try {
            ...
            SOPError("Error n RetrunSelectArrVal: " + ex);
            return "";
        }
    }
}
