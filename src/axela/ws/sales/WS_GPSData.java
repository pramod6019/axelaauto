...
public class WS_GPSData extends Connect {
	...
	public String gps_comp_id = "";
	public String active = "";
	public String comp_id = "0";
	public double lat = 0, lan = 0;
	JSONObject output = new JSONObject();

	public JSONObject gpsdata(JSONObject input) throws Exception {
		try {
			if (AppRun().equals("0")) {
				SOP("input = " + input);
			}
			...
			if (!input.isNull("gps_emp_id")) {
				gps_emp_id = CNumeric(PadQuotes((String) input
						.get("gps_emp_id")));
			...
			active = ExecuteQuery("SELECT emp_active" + " FROM "
					...
					+ gps_emp_id + "");
			if (isNumeric(gps_latitude) && !gps_latitude.equals("")
					&& isNumeric(gps_longitude) && !gps_longitude.equals("")
					&& active.equals("1")) {
				if (lat != 0 && lan != 0) {
					AddFields();
				} else {
		...
		try {
			...
			if (msg.equals("")) {
				StrSql = "INSERT INTO " + compdb(comp_id) + "axela_emp_gps"
						+ " (gps_emp_id," + " gps_latitude,"
						+ " gps_longitude," + " gps_time)" + " VALUES" + " ("
						+ gps_emp_id + "," + " " + gps_latitude + "," + " "
						+ gps_longitude + "," + " " + ToLongDate(kknow()) + ")";
				updateQuery(StrSql);
