...
public class TestClientTestDrive_List {
	...
	public static void main(String[] args) {
		...
		try {
			...
			URL url = new URL("http://localhost:6010/axelaauto-dc/mobile/sales/testdrive-list");
			conn = (HttpURLConnection) url.openConnection();
			conn.setDoOutput(true);
			conn.setRequestMethod("POST");
			conn.setRequestProperty("Accept", "application/json");
			conn.setRequestProperty("Content-Type", "application/json");
			String input = "{\"emp_id\":\"2\",\"startdate\":\"26/03/2014\"}";
			...
			while ((output = br.readLine()) != null) {
				...
				try {
					...
					Logger.getLogger(TestClientTestDrive_List.class.getName()).log(Level.SEVERE, null, ex);
				}
			}
			conn.disconnect();
			br.close();
