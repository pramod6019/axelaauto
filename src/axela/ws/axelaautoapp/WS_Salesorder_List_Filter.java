...
public class WS_Salesorder_List_Filter extends Connect {
	...
	Gson gson = new Gson();
	public JSONObject SalesorderListFilter(JSONObject input, @Context HttpServletRequest request) throws Exception {
		try {
			if (AppRun().equals("0")) {

				// SOP("input salesorderlistfilter for testing ======== " + input);
			}
	...
	public JSONObject PopulateBranchName(String branch_id, HttpServletRequest request) {
		...
		try {
			...
			// SOP("SqlStr==PopulateBranchName=" + SqlStr);
