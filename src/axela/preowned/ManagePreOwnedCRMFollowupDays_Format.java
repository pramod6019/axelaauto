...
public class ManagePreOwnedCRMFollowupDays_Format extends Connect {
	...
	public String title = "";
	public String precrmfollowupdays_id = "";
		...
		try {
			...
			if (!comp_id.equals("0")) {
				...
				if ("Update".equals(updateB)) {
					...
					if (msg.equals("")) {
						UpdateFields();
						response.sendRedirect(response.encodeRedirectURL("managepreownedcrmfollowupdays-update.jsp?update=yes&precrmfollowupdays_id=" + precrmfollowupdays_id
								+ "&msg=Format details for " + status + " updated successfully!"));
					}
				}
			}
		} catch (Exception ex) {
	...
	protected void PopulateFields() {
		...
		StrSql = StrSql + " FROM " + compdb(comp_id) + "axela_preowned_crmfollowupdays where precrmfollowupdays_id=" + precrmfollowupdays_id;
	...
	protected void UpdateFields() {
		...
		try {
			StrSql = "UPDATE " + compdb(comp_id) + "axela_preowned_crmfollowupdays Set "
					+ "" + FormatName + " = '" + format_desc + "' ";
			if (email.equals("yes")) {
				StrSql = StrSql + "," + subjectName + " = '" + subject + "'";
			}
			StrSql = StrSql + " where precrmfollowupdays_id=" + precrmfollowupdays_id;
			// SOP("SqlStr------------" + StrSql);
			updateQuery(StrSql);
		} catch (Exception ex) {
			SOPError("Axelaauto== " + this.getClass().getName());
