...
public class Preowned_Followup_Esc_Dashboard extends Connect {
	...
	public String PreownedEscDashboard() {
		try {
			...
			StrSql = "SELECT branch_id, branch_name, "
					+ " SUM(IF(preownedfollowup_trigger = 1, 1, 0)) AS level1, "
					+ " SUM(IF(preownedfollowup_trigger = 2, 1, 0)) AS level2, "
					+ " SUM(IF(preownedfollowup_trigger = 3, 1, 0)) AS level3, "
					+ " SUM(IF(preownedfollowup_trigger = 4, 1, 0)) AS level4, "
					+ " SUM(IF(preownedfollowup_trigger = 5, 1, 0)) AS level5, "
					+ " COUNT(preownedfollowup_id) AS preownedcount "
			...
			StrSql += " AND preownedfollowup_desc = ''"
