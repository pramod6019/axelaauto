...
public class Preowned_TestDrive_Fb_Pending extends Connect {
		...
		try {
			...
			if (!comp_id.equals("0"))
			{
				...
				if (go.equals("Go")) {
					...
					if (msg.equals("")) {
						StrHTML = ListTestDriveFbPending();
					}
				}
			}
	...
	public String ListTestDriveFbPending() {
		...
		StrSql += " WHERE testdrive_fb_taken = '0' "
				+ " AND testdrive_mileage_entry_id != 0 "
				+ " AND branch_active = '1' " + StrSearch
				+ " AND enquiry_branch_id IN ( " + branch_id + ")"
				+ " GROUP BY testdrive_id "
				+ " ORDER BY emp_name, testdrive_time limit 500";
		// SOP("ListTestDriveFbPending===" + StrSql);
		try {
			...
			if (crs.isBeforeFirst()) {
				...
				while (crs.next()) {
					...
					Str.append(crs.getString("testdrive_id")).append(">").append(crs.getString("testdrive_id"));
					Str.append("</a></td>");
				...
				Str.append("<br><br><center><font color=red><b>No Test Drive Feedback Pending</b></font></center>");
			}
			crs.close();
