...
public class Report_Insurance_SOE_Dash extends Connect {
		...
		try {
			...
			if (!comp_id.equals("0")) {
				...
				if (go.equals("Go")) {
					...
					if (msg.equals("")) {
						StrHTML = InsuranceSOEBookingSummary();
					}
				}
			}
		} catch (Exception ex) {
	...
	public String InsuranceSOEBookingSummary() {
		...
		try {
			StrSql = " SELECT "
					...
					+ " INNER JOIN " + compdb(comp_id) + "axela_insurance_enquiry ON insurenquiry_soe_id = soe_id"
