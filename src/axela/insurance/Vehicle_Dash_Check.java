...
import axela.service.Vehicle_Update;
...
public class Vehicle_Dash_Check extends Connect {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!comp_id.equals("0")) {
			...
			vehstock_status = PadQuotes(request.getParameter("vehstock_status"));
			/* ........... */
			...
			} else if (list_model_item.equals("yes")) {
				StrHTML = veh_dash.PopulateItem(comp_id, item_model_id);
			...
			} else if (model.equals("yes") || !veh_branch_id.equals("0")) {
				StrHTML = new Vehicle_Update().PopulateModel(comp_id, veh_branch_id);
			}

			if (vehstock_status.equals("yes") && !item_name.equals("")) {
			...
			try {
				if (ReturnPerm(comp_id, "emp_service_vehicle_add", request).equals("1")) {
					...
					} else if (name.equals("dr_item_id")) {
						if (!value.equals("0")) {
							value = value.replaceAll("nbsp", "&");
							history_oldvalue = ExecuteQuery("SELECT item_name FROM " + compdb(comp_id) + "axela_service_veh"
					...
					} else if (name.equals("txt_veh_purchase_amount")) {
						if (!value.equals("")) {
							value = value.replaceAll("nbsp", "&");
					...
					// //End of Last Service Kms
					else if (name.equals("dr_veh_insuremp_id")) {
						if (!value.equals("0")) {
							value = value.replaceAll("nbsp", "&");
							history_oldvalue = (ExecuteQuery("SELECT emp_name"
									+ " FROM " + compdb(comp_id) + "axela_emp"
									+ " INNER JOIN " + compdb(comp_id) + "axela_service_veh ON veh_insuremp_id = emp_id"
									+ " WHERE veh_id = " + veh_id));

							StrSql = "UPDATE " + compdb(comp_id) + "axela_service_veh"
									+ " SET"
									+ " veh_insuremp_id = '" + CNumeric(value) + "'"
									+ " WHERE veh_id = " + veh_id + "";
							updateQuery(StrSql);
							...
							history_actiontype = "Insurance Executive";
							...
							StrHTML = "<font color=\"red\">Select Insurance Executive!</font>";
						}
						...
						if (!value.equals("0")) {
							...
							StrSql = "INSERT INTO " + compdb(comp_id) + "axela_service_veh_history"
									...
									+ " '" + history_newvalue + "')";
							updateQuery(StrSql);
							StrHTML = "Service Executive updated!";
						} else {
							StrHTML = "<font color=\"red\">Select Service Executive!</font>";
						}
					} else if (name.equals("dr_veh_insursource_id")) {
						if (!value.equals("")) {
							value = value.replaceAll("nbsp", "&");
							history_oldvalue = (ExecuteQuery("SELECT insursource_name"
									+ " FROM " + compdb(comp_id) + "axela_service_veh"
									+ " INNER JOIN " + compdb(comp_id) + "axela_insurance_source ON insursource_id = veh_insursource_id"
									+ " WHERE veh_id = " + veh_id));

							StrSql = "UPDATE " + compdb(comp_id) + "axela_service_veh"
									+ " SET"
									+ " veh_insursource_id = '" + CNumeric(value) + "'"
									+ " WHERE veh_id = " + veh_id + "";
							updateQuery(StrSql);

							history_newvalue = ExecuteQuery("SELECT insursource_name"
									+ " FROM " + compdb(comp_id) + "axela_service_veh"
									+ " INNER JOIN " + compdb(comp_id) + "axela_insurance_source ON insursource_id = veh_insursource_id"
									+ " WHERE veh_insursource_id = " + value);

							history_actiontype = "Insurance Source";
							...
							StrSql = "INSERT INTO " + compdb(comp_id) + "axela_service_veh_history"
									...
									+ " '" + history_newvalue + "')";
							updateQuery(StrSql);
							StrHTML = "Insurance Source updated!";
						}
					} else if (name.equals("txt_veh_renewal_date")) {
						if ((!value.equals("") && isValidDateFormatShort(value)) || value.equals("")) {
							value = value.replaceAll("nbsp", "&");
							history_oldvalue = strToShortDate(ExecuteQuery("SELECT veh_renewal_date"
									+ " FROM " + compdb(comp_id) + "axela_service_veh"
									+ " WHERE veh_id = " + veh_id));
							...
							if (!value.equals("") && isValidDateFormatShort(value)) {
								StrSql = "UPDATE " + compdb(comp_id) + "axela_service_veh"
										+ " SET"
										+ " veh_renewal_date = '" + Long.parseLong(ConvertShortDateToStr(value)) + "'"
										+ " WHERE veh_id = " + veh_id + "";
								updateQuery(StrSql);
							} else {
								StrSql = "UPDATE " + compdb(comp_id) + "axela_service_veh"
							...
							history_newvalue = strToShortDate(ExecuteQuery("SELECT veh_renewal_date"
									+ " FROM " + compdb(comp_id) + "axela_service_veh"
									+ " WHERE veh_id = " + veh_id));
					...
					} else if (name.equals("dr_veh_insurtype_id")) {
						// SOP("1");
						if (!value.equals("0")) {
							value = value.replaceAll("nbsp", "&");
							history_oldvalue = (ExecuteQuery("SELECT insurtype_name"
									+ " FROM " + compdb(comp_id) + "axela_insurance_type"
									+ " INNER JOIN " + compdb(comp_id) + "axela_service_veh ON veh_insurtype_id = insurtype_id"
									+ " WHERE veh_id = " + veh_id));
							// SOP("2");
							StrSql = "UPDATE " + compdb(comp_id) + "axela_service_veh"
									+ " SET"
									+ " veh_insurtype_id = '" + CNumeric(value) + "'"
									+ " WHERE veh_id = " + veh_id + "";
							updateQuery(StrSql);
							// SOP("3");
							history_newvalue = (ExecuteQuery("SELECT insurtype_name"
									+ " FROM " + compdb(comp_id) + "axela_insurance_type"
									// + " INNER JOIN " + compdb(comp_id) + "axela_service_veh ON veh_insurtype_id = insurtype_id"
	...
	public String StockOnHandDetails() {
		...
		try {
			StrSql = "SELECT item_id, item_code, item_name, uom_name, vehstock_reorderlevel,"
					+ " vehstock_current_qty, vehstock_stockinorder, vehstock_stockindemand,"
					+ " vehstock_unit_cost, vehstock_onhandvalue, item_eoq"
					...
					+ " INNER JOIN " + compdb(comp_id) + "axela_inventory_stock ON vehstock_item_id = item_id"
					+ " INNER JOIN " + compdb(comp_id) + "axela_inventory_location ON location_id = vehstock_location_id"
			...
			if (crs.isBeforeFirst()) {
				...
				while (crs.next()) {
					...
					short_fall = "";
					order = "";
					nett_available = crs.getDouble("vehstock_current_qty") + crs.getDouble("vehstock_stockinorder") - crs.getDouble("vehstock_stockindemand");
					short_fall = Double.toString(crs.getDouble("vehstock_reorderlevel") - nett_available);
					if (Double.parseDouble(CNumeric(short_fall)) < 1) {
						short_fall = "";
					}

					if (!short_fall.equals("")) {
						if (Double.parseDouble(CNumeric(short_fall)) > crs.getDouble("vehstock_reorderlevel")) {
							order = short_fall;
						} else {
							order = crs.getString("vehstock_reorderlevel");
						}
					}
					...
					Str.append("<td align=\"right\" valign=\"top\">").append(crs.getDouble("vehstock_unit_cost")).append("</td>\n");
					Str.append("<td align=\"right\" valign=\"top\">").append(crs.getDouble("vehstock_onhandvalue")).append("</td>\n");
					Str.append("<td align=\"right\" valign=\"top\">").append(crs.getDouble("vehstock_current_qty")).append("</td>\n");
					Str.append("<td align=\"right\" valign=\"top\">").append(crs.getString("vehstock_stockinorder")).append("</td>\n");
					Str.append("<td align=\"right\" valign=\"top\">").append(crs.getString("vehstock_stockindemand")).append("</td>\n");
					Str.append("<td align=\"right\" valign=\"top\">").append(nett_available).append("</td>\n");
					Str.append("<td align=\"right\" valign=\"top\">").append(crs.getString("vehstock_reorderlevel")).append("</td>\n");
