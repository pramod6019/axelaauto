...
public class ManageLocation_Update extends Connect {
    ...
    public String location_inspection_dur = "";
            ...
            if ("yes".equals(add)) {
                ...
                if (!"yes".equals(addB)) {
                    ...
                    if (!msg.equals("")) {
                        ...
                        response.sendRedirect(response.encodeRedirectURL("managelocation.jsp?location_id=" + location_id + "&msg=Location Added Successfully!"));
                    }
                }
                ...
                if (!"yes".equals(updateB) && !"Delete Location".equals(deleteB)) {
                    ...
                    if (!msg.equals("")) {
                        ...
                        response.sendRedirect(response.encodeRedirectURL("managelocation.jsp?location_id=" + location_id + "&msg=Location Updated Successfully!"));
                    }
                    ...
                    if (!msg.equals("")) {
                        ...
                        response.sendRedirect(response.encodeRedirectURL("managelocation.jsp?msg=Location Deleted Successfully!"));
                    }
                }
            }
        }
    ...
    protected void GetValues(HttpServletRequest request,
        ...
        location_inspection_dur = PadQuotes(request.getParameter("txt_location_inspection_dur"));
        location_active = CheckBoxValue(PadQuotes(request.getParameter("chk_location_active")));
    ...
    protected void CheckForm() {
        ...
        if (location_name.equals("")) {
            ...
            StrSql = "SELECT location_id FROM " + compdb(comp_id) + "axela_service_location"
                    + " WHERE location_name = " + location_name + ""
                    + " AND location_branch_id = " + location_branch_id + "";
            if (ExecuteQuery(StrSql).equals(location_name)) {
                msg = msg + "<br>Similar Location found!";
            }
        }
        ...
        if (location_inspection_dur.equals("")) {
            msg = msg + "<br>Enter the Inspection Duration!";
        }
    }
    ...
    protected void AddFields() {
        CheckForm();
        if (msg.equals("")) {
            location_id = ExecuteQuery("SELECT (COALESCE(MAX(location_id), 0) + 1) FROM " + compdb(comp_id) + "axela_service_location");
            StrSql = "INSERT INTO " + compdb(comp_id) + "axela_service_location"
                    + " (location_id,"
                    + " location_name,"
                    + " location_branch_id,"
                    + " location_leadtime,"
                    + " location_inspection_dur,"
                    + " location_active,"
                    + " location_entry_id,"
                    + " location_entry_date)"
                    + " VALUES"
    ...
    protected void PopulateFields(HttpServletResponse response) {
        try {
            StrSql = "SELECT location_name, location_branch_id, location_leadtime,"
                    + " location_inspection_dur, location_active, location_entry_id,"
                    + " location_entry_date, location_modified_id, location_modified_date"
            ...
            if (crs.isBeforeFirst()) {
                while (crs.next()) {
                    ...
                    location_inspection_dur = crs.getString("location_inspection_dur");
    ...
    protected void UpdateFields() {
        CheckForm();
        if (msg.equals("")) {
            StrSql = "UPDATE " + compdb(comp_id) + "axela_service_location"
                    + " SET"
                    + " location_name = '" + location_name + "',"
                    + " location_branch_id = " + location_branch_id + ","
                    + " location_leadtime = " + location_leadtime + ","
                    + " location_inspection_dur = " + location_inspection_dur + ","
    ...
    protected void DeleteFields() {
        StrSql = "SELECT pickup_location_id FROM " + compdb(comp_id) + "axela_service_pickup"
                + " WHERE pickup_location_id = " + location_id + "";
        if (ExecuteQuery(StrSql).equals(location_id)) {
            msg += "<br>Location is associated with Pickup!";
        }

        if (msg.equals("")) {
            StrSql = "DELETE FROM " + compdb(comp_id) + "axela_service_location"
                    + " WHERE location_id = " + location_id + "";
            updateQuery(StrSql);
        }
    }
