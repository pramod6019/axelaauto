...
public class Vehicle_Dash_Jobcard extends Connect {
	...
	public String ListData(String comp_id, String veh_id) {
		...
		StrSql = "SELECT jc_id, jc_branch_id, CONCAT('JC', branch_code, jc_no) AS jc_no, jc_time_in,"
				...
				+ " jcstage_name, preownedmodel_name, variant_name, veh_id, veh_reg_no, priorityjc_name, jccat_name,"
				...
				+ " CONCAT(emp_name, ' (', emp_ref_no, ')') AS emp_name, emp_id, variant_id, preownedmodel_id";
		...
		if (TotalRecords != 0) {
			...
			try {
				...
				Str.append("<th>").append(GridLink("Item", "variant_name", ordernavi, ordertype)).append("</th>\n");
				...
				while (crs.next()) {
					...
					Str.append("</td>\n<td align=\"left\" valign=\"top\"><a href=\"../preowned/managepreownedvariant.jsp?variant_id=").append(crs.getString("variant_id")).append("\">");
