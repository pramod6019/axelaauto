...
public class ManagePSFDays_Format extends Connect {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (!comp_id.equals("0")) {
				...
				psfdays_id = PadQuotes(request.getParameter("psfdays_id"));
				...
				if ("Update".equals(updateB)) {
					...
					if (msg.equals("")) {
						UpdateFields();
						response.sendRedirect(response.encodeRedirectURL("psfdays-update.jsp?update=yes&psfdays_id=" + psfdays_id + "&msg=Format details for " + status + " updated successfully!"));
					}
				}
			}
		} catch (Exception ex) {
	...
	protected void PopulateFields() {
		...
		StrSql = StrSql + " FROM " + compdb(comp_id) + "axela_service_jc_psfdays where psfdays_id=" + psfdays_id;
	...
	protected void UpdateFields() {
		...
		try {
			StrSql = "UPDATE " + compdb(comp_id) + "axela_service_jc_psfdays SET "
					+ "" + FormatName + " = '" + format_desc + "' ";
			if (email.equals("yes")) {
				StrSql = StrSql + "," + subjectName + " = '" + subject + "'";
			}
			StrSql = StrSql + " where psfdays_id=" + psfdays_id;
			// SOP("SqlStr==" + StrSql);
			updateQuery(StrSql);
		} catch (Exception ex) {
			SOPError("Axelaauto== " + this.getClass().getName());
