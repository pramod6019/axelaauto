...
public class Report_JobCard_Stage_Esc_Status extends Connect {
        ...
        try {
            ...
            for (int i = 1; i <= 5; i++) {
                StrSql = " SELECT jc_id, concat(emp_name,' (',emp_ref_no,')') as emp_name "
                        ...
                        + " where 1=1 "
                        + " and jc_active = '1' and jc_stage_trigger=" + i + " and jc_time_ready='' and jc_time_in <= " + ToLongDate(kknow())
...
//                        SOP("StrSql===" + StrSqlBreaker(StrSql));
                ...
                if (crs2.isBeforeFirst()) {
                    ...
                    while (crs2.next()) {
                        Str.append("<a href=jobcard-list.jsp?jc_id=").append(crs2.getString("jc_id")).append(">").append(crs2.getString("jc_id")).append(": ").append(crs2.getString("emp_name")).append("</a><br>");
                    }
                    Str.append("</td>");
                } else {
    ...
    public String getDepartment() {
        ...
        try {
            ...
            if (crs.isBeforeFirst()) {
                int count = 0;
                Str.append("<table border=1 cellspacing=0 cellpadding=0 class=\"listtable\">");
                Str.append("<tr><th colspan=9>Job Card Stage</th></tr>");
                Str.append("<tr align=center>\n");
