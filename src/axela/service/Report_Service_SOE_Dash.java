...
public class Report_Service_SOE_Dash extends Connect {
	...
	public String SearchURL = "report-service-soe-dash.jsp?";
	...
	public String ServiceBookingSummary() {
		...
		int count = 0;
		StringBuilder soe_ids = new StringBuilder();
		try {
			StrSql = " SELECT "
					+ " soe_id,"
			...
			StrSql += StrSearch
					...
					+ " AND vehfollowup_soe_id = soe_id), 0) AS serviced ,"

					+ " COALESCE(soe_name, '') AS soename"
					+ " FROM " + compdb(comp_id) + "axela_soe"
					+ " INNER JOIN " + compdb(comp_id) + "axela_service_followup ON soe_id = vehfollowup_soe_id"
					+ " INNER JOIN " + compdb(comp_id) + "axela_service_veh ON veh_id = vehfollowup_veh_id"
			...
			if (crs.isBeforeFirst()) {
				...
				while (crs.next()) {
					...
					soe_ids = soe_ids.append(crs.getString("soe_id") + ",");
					Str.append("<tr align=left valign=top>\n");
					Str.append("<td align=center>").append(count).append("</td>");
					Str.append("<td align=center>").append(crs.getString("soename")).append("</td>\n");
					Str.append("<td valign=top align=right><a href=").append(SearchURL).append("filter=yes&enquiry=yes&soe_id=");
					Str.append(crs.getString("soe_id") + "&").append(multiSelect + ">").append(crs.getString("enquiry")).append("</a></td>");
					Str.append("<td valign=top align=right><a href=").append(SearchURL).append("filter=yes&serviced=yes&soe_id=");
					Str.append(crs.getString("soe_id") + "&").append(multiSelect + ">").append(crs.getString("serviced")).append("</a></td>");
					Str.append("</tr>");
				}
				Str.append("<tr align=center>\n");
				Str.append("<td colspan='2' align=right><b>Total:</b></td>");
				Str.append("<td valign=top align=right><a href=").append(SearchURL).append("filter=yes&enquiry=yes&soe_ids=" + soe_ids);
				Str.append(multiSelect + "><b>").append(enquiry_total).append("</b></a></td>");
				Str.append("<td valign=top align=right><a href=").append(SearchURL).append("filter=yes&serviced=yes&soe_ids=" + soe_ids);
	...
	private void SoeDetails(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			StrSearch += " AND SUBSTR(vehfollowup_enquiry_time, 1, 8) >= SUBSTR('" + starttime + "', 1, 8)"
					+ " AND SUBSTR(vehfollowup_enquiry_time, 1, 8) <= SUBSTR('" + endtime + "', 1, 8)";
			if (!soe_id.equals("")) {
				StrSearch += " AND vehfollowup_soe_id = " + soe_id;
			}

			if (!soe_ids.equals("")) {
				StrSearch += " AND vehfollowup_soe_id IN (" + soe_ids + ")";
			}
