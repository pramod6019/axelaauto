...
public class Report_JobCard_Priority_Esc_Dashboard extends Connect {
    ...
    public String priorityjc_id = "", exe_id = "";
    public String StrSearch = "";
    public String[] jc_priorityjc_id, jc_emp_id;
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (go.equals("Go")) {
                if (!priorityjc_id.equals("")) {
                    StrSearch = StrSearch + " and jc_priorityjc_id in (" + priorityjc_id + ")";
                }
                if (!exe_id.equals("")) {
    ...
    protected void GetValues(HttpServletRequest request,
        ...
        priorityjc_id = RetrunSelectArrVal(request, "dr_priority");
        jc_priorityjc_id = request.getParameterValues("dr_priority");
    ...
    public String JobCardEscDashboard() {
        try {
            ...
            StrSql = "select branch_id, branch_name, "
                    + " sum(if(jc_priority_trigger=1,1,0)) as level1, "
                    + " sum(if(jc_priority_trigger=2,1,0)) as level2, "
                    + " sum(if(jc_priority_trigger=3,1,0)) as level3, "
                    + " sum(if(jc_priority_trigger=4,1,0)) as level4, "
                    + " sum(if(jc_priority_trigger=5,1,0)) as level5, "
                    + " count(jc_id) as jobcardcount "
                    + " from " + compdb(comp_id) + "axela_branch "
                    + " left join " + compdb(comp_id) + "axela_service_jc on jc_branch_id = branch_id "
                    + " AND jc_active='1'"
                    + " and jc_priority_trigger > 0 and jc_time_ready='' and jc_time_promised < " + ToLongDate(kknow())
                    + ExeAccess.replace("emp_id", "jc_emp_id") + BranchAccess.replace("branch_id", "jc_branch_id")
                    + " where 1=1 " + StrSearch
                    + " group by branch_id ";
//            SOP("StrSql===" + StrSqlBreaker(StrSql));
    ...
    public String PopulatePriority() {
        ...
        try {
            ...
            CachedRowSet crs = processQuery(StrSql, 0);
            while (crs.next()) {
                Str.append("<option value=").append(crs.getString("priorityjc_id")).append("");
                Str.append(ArrSelectdrop(crs.getInt("priorityjc_id"), jc_priorityjc_id));
